import { r as __toESM$1, t as __commonJSMin } from "./chunk-B-1-B7_t.js";
import { p as isAxiosError, v as axios } from "./axios-DRI3k8XS.js";
...
//#endregion
//#region node_modules/stream-chat/dist/esm/index.mjs
...
var require_https = __commonJS({ "(disabled):https"() {} });
var require_jsonwebtoken = __commonJS({ "(disabled):node_modules/jsonwebtoken/index.js"() {} });
var require_crypto = __commonJS({ "(disabled):crypto"() {} });
var require_zlib = __commonJS({ "(disabled):zlib"() {} });
...
function map(arrayOrString, callback) {
	...
	if (isString(arrayOrString) && isMapStringCallback(arrayOrString, callback)) {
		for (let k = 0, len = arrayOrString.length; k < len; k++) if (arrayOrString.charAt(k)) res[k] = callback(arrayOrString.charAt(k), k, arrayOrString);
	} else if (!isString(arrayOrString) && !isMapStringCallback(arrayOrString, callback)) {
		for (let k = 0, len = arrayOrString.length; k < len; k++) if (k in arrayOrString) {
			const kValue = arrayOrString[k];
			res[k] = callback(kValue, k, arrayOrString);
		}
	}
	return res;
}
...
var decodeBase64 = (s) => {
	...
	for (x = 0; x < L; x++) {
		...
		b = (b << 6) + c;
		l += 6;
		while (l >= 8) ((a = b >>> (l -= 8) & 255) || x < L - 2) && (r += w(a));
	}
	return r;
};
var Campaign = class {
	...
	verifyCampaignId() {
		if (!this.id) throw new Error("Campaign id is missing. Either create the campaign using campaign.create() or set the id during instantiation - const campaign = client.campaign(id)");
	}
	async start(options) {
...
function addFileToFormData(uri, name, contentType) {
	const data = new import_browser.default();
	if (isReadableStream(uri) || isBuffer(uri) || isFileWebAPI(uri) || isBlobWebAPI(uri)) if (name) data.append("file", uri, name);
...
function normalizeQuerySort(sort) {
	...
	for (const item of sortArr) {
		...
		if (entries.length > 1) console.warn("client._buildSort() - multiple fields in a single sort object detected. Object's field order is not guaranteed");
		for (const [field, direction] of entries) sortFields.push({
			field,
			direction
		});
	}
...
function getRandomValuesWithMathRandom(bytes) {
	...
	for (let i = 0; i < bytes.length; i++) bytes[i] = Math.random() * max;
}
var getRandomValues = (() => {
	if (typeof crypto !== "undefined" && typeof crypto?.getRandomValues !== "undefined") return crypto.getRandomValues.bind(crypto);
	else if (typeof msCrypto !== "undefined") return msCrypto.getRandomValues.bind(msCrypto);
	else return getRandomValuesWithMathRandom;
})();
...
function convertErrorToJson(err) {
	...
	try {
		...
		return { error: "failed to serialize the error" };
	}
	return jsonObj;
}
function isOnline() {
	...
	if (typeof nav.onLine !== "boolean") return true;
	return nav.onLine;
}
...
var axiosParamsSerializer = (params) => {
	...
	for (const k in params) {
		...
		if (Array.isArray(params[k]) || typeof params[k] === "object") newParams.push(`${k}=${encodeURIComponent(JSON.stringify(params[k]))}`);
		else newParams.push(`${k}=${encodeURIComponent(params[k])}`);
	}
...
function formatMessage(message) {
	const toLocalMessageBase = (msg) => {
		...
		return {
			...
			reaction_groups: maybeGetReactionGroupsFallback(msg.reaction_groups, msg.reaction_counts, msg.reaction_scores),
...
var localMessageToNewMessagePayload = (localMessage) => {
	const { created_at, updated_at, deleted_at, error, status, latest_reactions, own_reactions, reaction_counts, reaction_scores, reply_count, command, html, i18n, quoted_message, mentioned_users, ...messageFields } = localMessage;
	return {
		...messageFields,
...
var toUpdatedMessagePayload = (message) => {
	...
	return {
		...Object.fromEntries(Object.entries(message).filter(([key]) => !reservedKeys[key])),
		pinned: !!message.pinned_at,
		mentioned_users: message.mentioned_users?.map((user) => typeof user === "string" ? user : user.id)
	};
};
var toDeletedMessage = ({ message, deletedAt, hardDelete = false }) => {
...
var deleteUserMessages = ({ messages, user, hardDelete = false, deletedAt }) => {
	for (let i = 0; i < messages.length; i++) {
		const message = messages[i];
		if (message.user?.id === user.id) messages[i] = message.type === "deleted" ? message : toDeletedMessage({
			message,
			hardDelete,
			deletedAt
		});
		if (messages[i].quoted_message && message.quoted_message?.user?.id === user.id) messages[i].quoted_message = message.quoted_message.type === "deleted" ? message.quoted_message : toDeletedMessage({
...
var findIndexInSortedArray = ({ needle, sortedArray, selectKey, selectValueToCompare = (e) => e, sortDirection = "ascending" }) => {
	...
	while (left <= right) {
		...
		if (sortDirection === "ascending" && comparableNeedle < comparableMiddle || sortDirection === "descending" && comparableNeedle >= comparableMiddle) right = middle - 1;
		else left = middle + 1;
	}
	if (selectKey) {
		const needleKey = selectKey(needle);
		const step = sortDirection === "ascending" ? -1 : 1;
		for (let i = left + step; 0 <= i && i < sortedArray.length && selectValueToCompare(sortedArray[i]) === comparableNeedle; i += step) if (selectKey(sortedArray[i]) === needleKey) return i;
	}
	return left;
};
function addToMessageList(messages, newMessage, timestampChanged = false, sortBy = "created_at", addIfDoesNotExist = true) {
	...
	if (timestampChanged) newMessages = newMessages.filter((message) => !(message.id && newMessage.id === message.id));
	if (newMessages.length === 0 && addMessageToList) return newMessages.concat(newMessage);
	else if (newMessages.length === 0) return newMessages;
	const messageTime = newMessage[sortBy].getTime();
	const messageIsNewest = newMessages.at(-1)[sortBy].getTime() < messageTime;
	if (messageIsNewest && addMessageToList) return newMessages.concat(newMessage);
	else if (messageIsNewest) return newMessages;
	...
	if (addMessageToList) newMessages.splice(insertionIndex, 0, newMessage);
	return newMessages;
}
function maybeGetReactionGroupsFallback(groups, counts, scores) {
	...
	if (counts && scores) {
		const fallback = {};
		for (const type of Object.keys(counts)) fallback[type] = {
			count: counts[type],
...
var debounce = (fn, timeout = 0, { leading = false, trailing = true } = {}) => {
	...
	const debouncedFn = (...args) => {
		...
		else if (leading) lastResult = fn(...args);
	...
	debouncedFn.flush = () => {
		if (runningTimeout) {
			...
			runningTimeout = null;
			if (argsForTrailingExecution) lastResult = fn(...argsForTrailingExecution);
		}
		return lastResult;
	};
	return debouncedFn;
...
function binarySearchByDateEqualOrNearestGreater(array, targetDate) {
	...
	while (left <= right) {
		...
		if (midDate.getTime() === targetDate.getTime()) return mid;
		else if (midDate.getTime() < targetDate.getTime()) left = mid + 1;
		else right = mid - 1;
	}
	return left;
}
var messagePaginationCreatedAtAround = ({ parentSet, requestedPageSize, returnedPage, filteredReturnedPage, messagePaginationOptions }) => {
	...
	if (wholePageHasNewerMessages) {
		...
		const [firstFilteredPageMsg, lastFilteredPageMsg] = [filteredReturnedPage[0], filteredReturnedPage.slice(-1)[0]];
		const [firstPageMsgIsFirstInSet, lastPageMsgIsLastInSet] = [firstFilteredPageMsg?.id && firstFilteredPageMsg.id === parentSet.messages[0]?.id, lastFilteredPageMsg?.id && lastFilteredPageMsg.id === parentSet.messages.slice(-1)[0]?.id];
...
var messagePaginationIdAround = ({ parentSet, requestedPageSize, returnedPage, filteredReturnedPage, messagePaginationOptions }) => {
	const newPagination = { ...parentSet.pagination };
	...
	let hasNext;
	const [firstFilteredPageMsg, lastFilteredPageMsg] = [filteredReturnedPage[0], filteredReturnedPage.slice(-1)[0]];
	const [firstPageMsgIsFirstInSet, lastPageMsgIsLastInSet] = [firstFilteredPageMsg?.id === parentSet.messages[0]?.id, lastFilteredPageMsg?.id === parentSet.messages.slice(-1)[0]?.id];
	...
	if ((requestedPageSize > parentSet.messages.length || parentSet.messages.length >= returnedPage.length) && requestedPageSize > returnedPage.length) {
		hasNext = hasPrev = false;
		updateHasPrev = updateHasNext = true;
	} else if (!returnedPage[midPoint]) return newPagination;
	else if (returnedPage[midPoint].id === id_around) hasPrev = hasNext = true;
	else {
		let targetMsg;
		...
		for (let i = 0; i < halves.length; i++) {
			...
			if (targetMsg && i === 0) hasPrev = false;
			if (targetMsg && i === 1) hasNext = false;
		}
	}
	if (updateHasPrev && typeof hasPrev !== "undefined") newPagination.hasPrev = hasPrev;
	if (updateHasNext && typeof hasNext !== "undefined") newPagination.hasNext = hasNext;
...
var messagePaginationLinear = ({ parentSet, requestedPageSize, returnedPage, filteredReturnedPage, messagePaginationOptions }) => {
	const newPagination = { ...parentSet.pagination };
	let hasPrev;
	let hasNext;
	const [firstFilteredPageMsg, lastFilteredPageMsg] = [filteredReturnedPage[0], filteredReturnedPage.slice(-1)[0]];
	...
	if (typeof queriedPrevMessages !== "undefined" || containsUnrecognizedOptionsOnly) hasPrev = hasMore;
	if (typeof queriedNextMessages !== "undefined") hasNext = hasMore;
...
var messageSetPagination = (params) => {
	...
	if (params.messagePaginationOptions?.created_at_around) return messagePaginationCreatedAtAround(params);
	else if (params.messagePaginationOptions?.id_around) return messagePaginationIdAround(params);
	else return messagePaginationLinear(params);
};
var WATCH_QUERY_IN_PROGRESS_FOR_CHANNEL = {};
var getAndWatchChannel = async ({ channel, client, id, members, options, type }) => {
...
var isChannelArchived = (channel) => {
	if (!channel) return false;
	return !!channel.state.membership?.archived_at;
};
...
var extractSortValue = ({ atIndex, sort, targetKey }) => {
	if (!sort) return null;
	let option = null;
	if (Array.isArray(sort)) option = sort[atIndex] ?? null;
	else {
		let index = 0;
...
var findLastPinnedChannelIndex = ({ channels }) => {
	...
	for (const channel of channels) {
		...
		if (typeof lastPinnedChannelIndex === "number") lastPinnedChannelIndex++;
		else lastPinnedChannelIndex = 0;
	}
	return lastPinnedChannelIndex;
};
var promoteChannel = ({ channels, channelToMove, channelToMoveIndexWithinChannels, sort }) => {
	...
	if (targetChannelAlreadyAtTheTop || considerPinnedChannels && isTargetChannelPinned) return channels;
	const newChannels = [...channels];
	if (targetChannelExistsWithinList) newChannels.splice(targetChannelIndex, 1);
	let lastPinnedChannelIndex = null;
	if (considerPinnedChannels) lastPinnedChannelIndex = findLastPinnedChannelIndex({ channels: newChannels });
...
var runDetached = (callback, options) => {
	...
	if (onSuccessCallback) promise = promise.then(onSuccessCallback);
...
var ChannelState = class {
	constructor(channel) {
		...
		*/
		this.updateUserMessages = (user) => {
			const _updateUserMessages = (messages, user2) => {
				for (let i = 0; i < messages.length; i++) {
					const m = messages[i];
					if (m.user?.id === user2.id) messages[i] = {
						...m,
						user: user2
					};
				}
			...
			for (const parentId in this.threads) _updateUserMessages(this.threads[parentId], user);
		...
		*/
		this.deleteUserMessages = (user, hardDelete = false, deletedAt) => {
			this.messageSets.forEach(({ messages }) => deleteUserMessages({
				messages,
				user,
				hardDelete,
				deletedAt: deletedAt ?? null
			}));
			for (const parentId in this.threads) deleteUserMessages({
		...
		*/
		this.findMessageSetByOldestTimestamp = (oldestTimestampMs) => {
			...
			while (lo < hi) {
				...
				if (msgSet.messages[0].created_at.getTime() <= oldestTimestampMs) hi = mid;
				else lo = mid + 1;
			}
			return lo;
		};
	...
	*/
	addMessagesSorted(newMessages, timestampChanged = false, initializing = false, addIfDoesNotExist = true, messageSetToAddToIfDoesNotExist = "current") {
		...
		for (let i = 0; i < messagesToAdd.length; i += 1) {
			if (messagesToAdd[i].shadowed && addIfDoesNotExist) {
				filteredMessageIds.push(messagesToAdd[i].id);
				continue;
			}
			const isMessageFormatted = messagesToAdd[i].created_at instanceof Date;
			let message;
			if (isMessageFormatted) message = messagesToAdd[i];
			else {
				message = this.formatMessage(messagesToAdd[i]);
				if (message.user && this._channel?.cid) this._channel.getClient().state.updateUserReference(message.user, this._channel.cid);
				if (initializing && message.id && this.threads[message.id] && !this._channel.getClient().preventThreadCleanup) delete this.threads[message.id];
				if (!(this._channel.getConfig()?.skip_last_msg_update_for_system_msgs && message.type === "system") && (!this.last_message_at || message.created_at.getTime() > this.last_message_at.getTime())) this.last_message_at = new Date(message.created_at.getTime());
			}
			const parentID = message.parent_id;
			if ((!parentID || message.show_in_channel) && targetMessageSetIndex !== -1) this.messageSets[targetMessageSetIndex].messages = this._addToMessageList(this.messageSets[targetMessageSetIndex].messages, message, timestampChanged, "created_at", addIfDoesNotExist);
	...
	*/
	addPinnedMessages(pinnedMessages) {
		for (let i = 0; i < pinnedMessages.length; i += 1) this.addPinnedMessage(pinnedMessages[i]);
	}
	/**
	...
	*/
	...
	addReaction(reaction, message, enforce_unique) {
		...
		if (!messageWithReaction) messageFromState = this.findMessage(reaction.message_id);
		if (!messageWithReaction && !messageFromState) return;
		...
		this._updateMessage(updateData, (msg) => {
			...
			if (messageFromState) return this._addReactionToState(messageFromState, reaction, enforce_unique);
			return msg;
		});
		return messageWithReaction ?? messageFromState;
	}
	_addReactionToState(messageFromState, reaction, enforce_unique) {
		if (!messageFromState.reaction_groups) messageFromState.reaction_groups = {};
		if (enforce_unique) for (const ownReaction of messageFromState.own_reactions ?? []) {
			...
			if (messageFromState.reaction_groups[ownReaction.type].count < 1) delete messageFromState.reaction_groups[ownReaction.type];
		}
		const oldReactionTypeData = messageFromState.reaction_groups[reaction.type];
	...
	_addOwnReactionToMessage(ownReactions, reaction, enforce_unique) {
		if (enforce_unique) ownReactions = [];
		else ownReactions = this._removeOwnReactionFromMessage(ownReactions, reaction);
		ownReactions = ownReactions || [];
		if (this._channel.getClient().userID === reaction.user_id) ownReactions.push(reaction);
		return ownReactions;
	}
	_removeOwnReactionFromMessage(ownReactions, reaction) {
		if (ownReactions) return ownReactions.filter((item) => item.user_id !== reaction.user_id || item.type !== reaction.type);
		return ownReactions;
	}
	removeReaction(reaction, message) {
		const messageWithRemovedReaction = message;
		let messageFromState;
		if (!messageWithRemovedReaction) messageFromState = this.findMessage(reaction.message_id);
		if (!messageWithRemovedReaction && !messageFromState) return;
		...
		this._updateMessage(updateData, (msg) => {
			...
			if (messageFromState) return this._removeReactionFromState(messageFromState, reaction);
			return msg;
		});
	...
	_removeReactionFromState(messageFromState, reaction) {
		...
		if (reactionToRemove && messageFromState.reaction_groups?.[reactionToRemove.type]) {
			...
			if (messageFromState.reaction_groups[reactionToRemove.type].count < 1) delete messageFromState.reaction_groups[reactionToRemove.type];
		}
	...
	_updateQuotedMessageReferences({ message, remove }) {
		...
		const update = (messages) => {
			const updatedMessages = messages.reduce((acc, msg) => {
				if (msg.quoted_message_id === message.id) acc.push({
					...parseMessage(msg),
		...
		if (!message.parent_id) this.messageSets.forEach((set) => update(set.messages));
		else if (message.parent_id && this.threads[message.parent_id]) update(this.threads[message.parent_id]);
	}
	...
	*/
	_updateMessage(message, updateFunc) {
		...
		if (pinned) {
			...
			if (msgIndex !== -1) this.pinnedMessages[msgIndex] = updateFunc(this.pinnedMessages[msgIndex]);
		}
	}
	/**
	...
	*/
	filterErrorMessages() {
		...
		for (const message of blockedMessages) this._channel.getClient().offlineDb?.hardDeleteMessage({ id: message.id });
	...
	*/
	async loadMessageIntoState(messageId, parentMessageId, limit = 25) {
		...
		if (messageId === "latest") {
			if (this.messages === this.latestMessages) return;
			messageSetIndex = this.messageSets.findIndex((s) => s.isLatest);
		} else messageSetIndex = this.findMessageSetIndex({ id: messageIdToFind });
		if (messageSetIndex !== -1) {
			this.switchToMessageSet(messageSetIndex);
			switchedToMessageSet = true;
		}
		loadedMessageThread = !parentMessageId || !!this.threads[parentMessageId]?.find((m) => m.id === messageId);
		if (switchedToMessageSet && loadedMessageThread) return;
		if (!switchedToMessageSet) await this._channel.query({ messages: {
			id_around: messageIdToFind,
			limit
		} }, "new");
		if (!loadedMessageThread && parentMessageId) await this._channel.getReplies(parentMessageId, {
			id_around: messageId,
			limit
		});
		messageSetIndex = this.findMessageSetIndex({ id: messageIdToFind });
		if (messageSetIndex !== -1) this.switchToMessageSet(messageSetIndex);
	}
	/**
	...
	*/
	...
	findMessageByTimestamp(timestampMs, parentMessageId, exactTsMatch = false) {
		if (parentMessageId && !this.threads[parentMessageId] || this.messageSets.length === 0) return null;
		...
		if ((timestampMs < firstMsgTimestamp || lastMsgTimestamp < timestampMs) && exactTsMatch) return null;
	...
	findTargetMessageSet(newMessages, addIfDoesNotExist = true, messageSetToAddToIfDoesNotExist = "current") {
		...
		if (newMessages.length === 0) return {
			targetMessageSetIndex: 0,
			messagesToAdd: newMessages
		};
		if (addIfDoesNotExist) {
			const overlappingMessageSetIndicesByMsgIds = this.messageSets.map((_, i) => i).filter((i) => this.areMessageSetsOverlap(this.messageSets[i].messages, newMessages));
			const overlappingMessageSetIndicesByTimestamp = this.messageSets.map((_, i) => i).filter((i) => messageSetsOverlapByTimestamp(this.messageSets[i].messages, newMessages.map(formatMessage)));
			switch (messageSetToAddToIfDoesNotExist) {
				case "new":
					if (overlappingMessageSetIndicesByMsgIds.length > 0) targetMessageSetIndex = overlappingMessageSetIndicesByMsgIds[0];
					else if (overlappingMessageSetIndicesByTimestamp.length > 0) targetMessageSetIndex = overlappingMessageSetIndicesByTimestamp[0];
					else if (newMessages.some((m) => !m.parent_id)) {
						const setIngestIndex = this.findMessageSetByOldestTimestamp(new Date(newMessages[0].created_at).getTime());
						if (setIngestIndex === -1) {
							...
							if (isLatest) this.messageSets.slice(1).forEach((set) => {
								set.isLatest = false;
							});
					...
					break;
				case "current":
					if (overlappingMessageSetIndicesByTimestamp.length > 0) targetMessageSetIndex = overlappingMessageSetIndicesByTimestamp[0];
					else targetMessageSetIndex = this.messageSets.findIndex((s) => s.isCurrent);
					break;
				case "latest":
					if (overlappingMessageSetIndicesByTimestamp.length > 0) targetMessageSetIndex = overlappingMessageSetIndicesByTimestamp[0];
					else targetMessageSetIndex = this.messageSets.findIndex((s) => s.isLatest);
					break;
				default: targetMessageSetIndex = -1;
			}
			const mergeTargetMessageSetIndex = overlappingMessageSetIndicesByMsgIds.splice(0, 1)[0];
			const mergeSourceMessageSetIndices = [...overlappingMessageSetIndicesByMsgIds];
			if (mergeTargetMessageSetIndex !== void 0 && mergeTargetMessageSetIndex !== targetMessageSetIndex) mergeSourceMessageSetIndices.push(targetMessageSetIndex);
			if (mergeSourceMessageSetIndices.length > 0) {
				...
				targetMessageSetIndex = this.messageSets.findIndex((s) => this.areMessageSetsOverlap(s.messages, newMessages));
			}
		} else targetMessageSetIndex = this.findMessageSetIndex(newMessages[0]);
		return {
...
var CooldownTimer = class extends WithSubscriptions {
	constructor({ channel }) {
		...
		this.registerSubscriptions = () => {
			...
			this.addUnsubscribeFunction(this.channel.on("message.new", (event) => {
				if (!(event.message?.user?.id && event.message.user.id === this.getOwnUserId())) return;
				this.setOwnLatestMessageDate(toDateOrUndefined(event.message?.created_at));
			}).unsubscribe);
			this.addUnsubscribeFunction(this.channel.on("channel.updated", (event) => {
				if (!(event.channel?.cooldown !== this.cooldownConfigSeconds)) return;
				this.refresh();
			}).unsubscribe);
		};
		...
		this.refresh = () => {
			...
			const ownLatestMessageDate = this.findOwnLatestMessageDate({ messages: this.channel.state.latestMessages });
			if (cooldownConfigSeconds !== this.cooldownConfigSeconds || ownLatestMessageDate?.getTime() !== this.ownLatestMessageDate?.getTime() || canSkipCooldown !== this.canSkipCooldown) this.state.partialNext({
			...
			if (this.canSkipCooldown || this.cooldownConfigSeconds === 0) {
				this.clearTimeout();
				if (this.cooldownRemaining !== 0) this.setCooldownRemaining(0);
				return;
			}
			this.recalculate();
		};
		...
		*/
		...
		this.recalculate = () => {
			...
			const timeSinceOwnLastMessage = ownLatestMessageDate != null ? Math.max(0, (Date.now() - ownLatestMessageDate.getTime()) / 1e3) : void 0;
			...
			if (remaining !== this.cooldownRemaining) this.setCooldownRemaining(remaining);
	...
	findOwnLatestMessageDate({ messages }) {
		...
		for (let i = messages.length - 1; i >= 0; i -= 1) {
			...
			if (!latest || createdAt.getTime() > latest.getTime()) latest = createdAt;
...
var ensureIsLocalAttachment = (attachment) => {
	...
	if (isLocalAttachment(attachment)) return attachment;
...
var createPostUploadAttachmentEnrichmentMiddleware = () => ({
	...
	handlers: { postProcess: ({ state, discard, forward, next }) => {
		...
		if (isLocalImageAttachment(attachment)) enrichedAttachment.image_url = response.file;
		else enrichedAttachment.asset_url = response.file;
		if (response.thumb_url) enrichedAttachment.thumb_url = response.thumb_url;
		return next({
			...state,
...
function wrapWithCancellation(tag, cb) {
	...
	const wrapped = () => {
		if (ac.signal.aborted) return Promise.resolve("canceled");
		return cb(ac.signal).finally(() => {
			if (!ac.signal.aborted) pendingPromises.delete(tag);
		});
	};
...
var MiddlewareExecutor = class {
	...
	replace(middleware) {
		...
		middleware.forEach((upserted) => {
			...
			if (existingIndex >= 0) newMiddleware.splice(existingIndex, 1, upserted);
			else newMiddleware.push(upserted);
		});
		this.middleware = newMiddleware;
		return this;
	}
	insert({ middleware, position, unique }) {
		if (unique) middleware.forEach((md) => {
			const existingMiddlewareIndex = this.middleware.findIndex((m) => m.id === md.id);
			if (existingMiddlewareIndex >= 0) this.middleware.splice(existingMiddlewareIndex, 1);
		});
	...
	remove(middlewareIds) {
		...
		this.middleware = this.middleware.filter((md) => typeof middlewareIds === "string" ? middlewareIds !== md.id : !middlewareIds.includes(md.id));
	}
	async executeMiddlewareChain({ eventName, initialValue, mode = "cancelable" }) {
		let index = -1;
		const execute = async (i, state, status) => {
			if (i <= index) throw new Error("next() called multiple times");
			index = i;
			if (i === this.middleware.length || status && ["complete", "discard"].includes(status)) return {
				state,
				status
			};
			const handler = this.middleware[i].handlers[eventName];
			if (!handler) return execute(i + 1, state, status);
		...
		const result = mode === "cancelable" ? await withCancellation(`middleware-execution-${this.id}-${eventName}`, async (abortSignal) => {
			const result2 = await execute(0, initialValue);
			if (abortSignal.aborted) return "canceled";
			return result2;
	...
	async execute({ eventName, initialValue: initialState, mode }) {
...
var createUploadErrorHandlerMiddleware = (composer) => ({
	...
	handlers: { postProcess: ({ state, discard, forward }) => {
...
var AttachmentPostUploadMiddlewareExecutor = class extends MiddlewareExecutor {
	constructor({ composer }) {
		super();
		this.use([createUploadErrorHandlerMiddleware(composer), createPostUploadAttachmentEnrichmentMiddleware()]);
	}
};
var createUploadConfigCheckMiddleware = (composer) => ({
	id: "stream-io/attachment-manager-middleware/file-upload-config-check",
	handlers: { prepare: async ({ state, next, discard }) => {
		const { attachmentManager } = composer;
		if (!attachmentManager || !state.attachment) return discard();
		const uploadPermissionCheck = await attachmentManager.getUploadConfigCheck(state.attachment.localMetadata.file);
		const attachment = {
			...state.attachment,
			localMetadata: {
...
var createBlockedAttachmentUploadNotificationMiddleware = (composer) => ({
	...
	handlers: { prepare: ({ state: { attachment }, forward }) => {
		if (!attachment) return forward();
		if (attachment.localMetadata.uploadPermissionCheck?.uploadBlocked) composer.client.notifications.addError({
			...
			options: {
				...
				metadata: { reason: attachment.localMetadata.uploadPermissionCheck?.reason }
			}
		});
		return forward();
	} }
...
var AttachmentPreUploadMiddlewareExecutor = class extends MiddlewareExecutor {
	constructor({ composer }) {
		super();
		this.use([createUploadConfigCheckMiddleware(composer), createBlockedAttachmentUploadNotificationMiddleware(composer)]);
	}
};
...
var isEqual = (value1, value2, compareStack = /* @__PURE__ */ new Set(), objectStack1 = /* @__PURE__ */ new WeakSet(), objectStack2 = /* @__PURE__ */ new WeakSet()) => {
	...
	const obj1 = value1;
	const obj2 = value2;
	if (objectStack1.has(obj1) || objectStack2.has(obj2)) return objectStack1.has(obj1) && objectStack2.has(obj2);
	...
	if (isArray1 && isArray2) {
		...
		for (let i = 0; i < arr1.length; i++) if (!isEqual(arr1[i], arr2[i], compareStack, objectStack1, objectStack2)) {
	...
	for (const key of keys2) if (!Object.prototype.hasOwnProperty.call(plainObj1, key)) {
		objectStack1.delete(obj1);
		objectStack2.delete(obj2);
		return false;
	}
	...
	for (const key of keys1) if (!isEqual(plainObj1[key], plainObj2[key], compareStack, objectStack1, objectStack2)) {
...
function compareAndBuildDiff(original, modified, parentDiffNode, key, compareStack = /* @__PURE__ */ new Set(), objectStack = /* @__PURE__ */ new Set()) {
	if (isEqual(original, modified, new Set(compareStack))) return;
	if (original === void 0 || original === null) {
		if (key !== void 0) parentDiffNode.children[String(key)] = {
			type: "added",
			value: modified,
			children: {}
		};
	...
	if (typeof original !== "object" || typeof modified !== "object" || original === null || modified === null || Array.isArray(original) !== Array.isArray(modified) || isClassInstance(original) || isClassInstance(modified)) {
		if (key !== void 0) parentDiffNode.children[String(key)] = {
			type: "updated",
			value: modified,
			oldValue: original,
	...
	if (key !== void 0) parentDiffNode.children[String(key)] = currentDiffNode;
	...
	for (const childKey of allKeys) {
		...
		if (!(childKey in modifiedObj)) continue;
...
function createMergeCore(options = {}) {
	...
	return function mergeCore({ target, source, customizer }) {
		...
		function createNewTarget(targetValue, srcValue) {
			if (targetValue === null || typeof targetValue === "undefined") return srcValue;
			if (!Array.isArray(targetValue) && typeof targetValue !== "object") return srcValue;
			if (targetValue && typeof targetValue === "object") {
				...
				if (isTargetClassInstance || isSourceClassInstance) {
					if (isSourceClassInstance) return srcValue;
					return targetValue;
				}
		...
		function processSourceValue(target2, src, sourceKey, stack, pendingMerges) {
			...
			if (handleCustomizer(targetValue, srcValue, sourceKey, target2, src, stack)) return;
			...
			} else if (srcValue !== void 0) target2[sourceKey] = srcValue;
		}
		function processKeys(target2, source2, stack, pendingMerges) {
			...
			for (const sourceKey of sourceKeys) processSourceValue(target2, source2, sourceKey, stack, pendingMerges);
		}
		function processPendingMerge({ target: target2, source: source2, sourceKey, parentTarget }, stack, pendingMerges) {
			if (stack.has(source2)) {
				if (trackDiff && sourceKey && parentTarget) Object.defineProperty(parentTarget, sourceKey, {
					value: target2,
					enumerable: true,
					writable: true,
					configurable: true
		...
		function baseMerge(object, source2, stack = /* @__PURE__ */ new Set()) {
			if (stack.has(object) || stack.has(source2)) return { ...object };
			const result2 = { ...object };
			const pendingMerges = [];
			stack.add(result2);
			stack.add(source2);
			processKeys(result2, source2, stack, pendingMerges);
			while (pendingMerges.length) processPendingMerge(pendingMerges.pop(), stack, pendingMerges);
			stack.delete(source2);
			stack.delete(result2);
			return result2;
		}
		const result = sources.reduce((result2, source2) => baseMerge(result2, source2), { ...target });
		return {
			result,
			diff: trackDiff && originalTarget ? generateDiff(originalTarget, result) : null
		};
	};
}
function cleanupDiffTree(diffNode) {
	...
	if (diffNode.type || hasChildren) return {
		...diffNode,
...
function mergeWithDiff(target, source, customizer) {
	const { result, diff } = createMergeCore({ trackDiff: true })({
		target,
		source,
		customizer
	});
	return {
		result,
		diff: cleanupDiffTree(diff ?? { children: {} }) || { children: {} }
	};
}
var initState = ({ message }) => ({ attachments: (message?.attachments ?? [])?.filter(({ og_scrape_url }) => !og_scrape_url).map((att) => {
...
var _AttachmentManager = class _AttachmentManager {
	constructor({ composer, message }) {
		...
		this.cancelAttachmentUploads = (attachments = this.attachments) => {
			for (const { localMetadata } of attachments) this.client.uploadManager.deleteUploadRecord(localMetadata.id);
		};
		...
		this.getSnapshot = () => {
			...
			for (let i = 0; i < attachments.length; i++) if (attachments[i] !== state.attachments[i]) {
				hasUpdates = true;
				break;
			}
			return hasUpdates ? {
		...
		this.clearAttachments = () => {
			...
			this.removeAttachments(this.attachments.map((attachment) => attachment.localMetadata.id));
		};
		...
		this.prepareAttachmentUpdate = (attachmentToUpdate) => {
			...
			if (merged.diff && Object.keys(merged.diff.children).length) {
		...
		this.updateAttachment = (attachmentToUpdate) => {
			...
			if (updatedAttachments && updatedAttachments !== this.attachments) this.state.partialNext({ attachments: updatedAttachments });
		};
		this.upsertAttachments = (attachmentsToUpsert) => {
			...
			if (hasUpdates) this.state.partialNext({ attachments });
		};
		this.removeAttachments = (localAttachmentIds) => {
			if (!localAttachmentIds.length) return;
			this.state.partialNext({ attachments: this.attachments.filter((attachment) => !localAttachmentIds.includes(attachment.localMetadata?.id)) });
			for (const id of localAttachmentIds) this.client.uploadManager.deleteUploadRecord(id);
		};
		this.getUploadConfigCheck = async (fileLike) => {
			const client = this.channel.getClient();
			let appSettings;
			if (!client.appSettingsPromise) appSettings = await client.getAppSettings();
			else appSettings = await client.appSettingsPromise;
			...
			const { allowed_file_extensions, allowed_mime_types, blocked_file_extensions, blocked_mime_types, size_limit } = uploadConfig;
			const sizeLimit = size_limit || DEFAULT_UPLOAD_SIZE_LIMIT_BYTES;
			const mimeType = fileLike.type;
			if (isFile(fileLike) || isFileReference(fileLike)) {
				if (allowed_file_extensions?.length && !allowed_file_extensions.some((ext) => fileLike.name.toLowerCase().endsWith(ext.toLowerCase()))) return {
					uploadBlocked: true,
					reason: "allowed_file_extensions"
				};
				if (blocked_file_extensions?.length && blocked_file_extensions.some((ext) => fileLike.name.toLowerCase().endsWith(ext.toLowerCase()))) return {
					uploadBlocked: true,
					reason: "blocked_file_extensions"
				};
			}
			if (allowed_mime_types?.length && !allowed_mime_types.some((type) => type.toLowerCase() === mimeType?.toLowerCase())) return {
				uploadBlocked: true,
				reason: "allowed_mime_types"
			};
			if (blocked_mime_types?.length && blocked_mime_types.some((type) => type.toLowerCase() === mimeType?.toLowerCase())) return {
				uploadBlocked: true,
				reason: "blocked_mime_types"
			};
			if (fileLike.size && fileLike.size > sizeLimit) return {
				uploadBlocked: true,
				reason: "size_limit"
			};
		...
		this.fileToLocalUploadAttachment = async (fileLike) => {
			...
			const uploadPermissionCheck = await this.getUploadConfigCheck(localAttachment.localMetadata.file);
		...
		this.ensureLocalUploadAttachment = async (attachment) => {
			...
			if (attachment.localMetadata.id) newAttachment.localMetadata.id = attachment.localMetadata.id;
		...
		*/
		this.doDefaultUploadRequest = async (fileLike, options) => {
			...
			if (isFileReference(fileLike)) return this.channel[isImageFile(fileLike) ? "sendImage" : "sendFile"](fileLike.uri, fileLike.name, fileLike.type, void 0, axiosUploadConfig);
		...
		*/
		this.doUploadRequest = async (fileLike, options) => {
			...
			if (customUploadFn) return await customUploadFn(fileLike, options);
		...
		this.uploadAttachment = async (attachment) => {
			...
			if (localAttachment.localMetadata.uploadState === "blocked") {
				...
				this.client.notifications.addError({
					...
					options: {
						type: "validation:attachment:upload:blocked",
						metadata: { reason: localAttachment.localMetadata.uploadPermissionCheck?.reason }
					}
				});
				return localAttachment;
			}
			...
			if (isLocalImageAttachment(uploadedAttachment)) uploadedAttachment.image_url = response.file;
			else uploadedAttachment.asset_url = response.file;
			if (response.thumb_url) uploadedAttachment.thumb_url = response.thumb_url;
		...
		this.uploadFile = async (file) => {
			const preUpload = await this.preUploadMiddlewareExecutor.execute({
				eventName: "prepare",
				initialValue: { attachment: _AttachmentManager.toLocalUploadAttachment(file) },
				mode: "concurrent"
			});
		...
		this.uploadFiles = async (files) => {
			...
			return await Promise.all(iterableFiles.slice(0, this.availableUploadSlots).map(this.uploadFile));
		};
		...
		this.preUploadMiddlewareExecutor = new AttachmentPreUploadMiddlewareExecutor({ composer });
		this.postUploadMiddlewareExecutor = new AttachmentPostUploadMiddlewareExecutor({ composer });
	}
	get attachmentsById() {
	...
	getUploadsByState(state) {
		return Object.values(this.attachments).filter(({ localMetadata }) => localMetadata.uploadState === state);
	}
	upload(attachment) {
		...
		const unsubscribe = this.client.uploadManager.state.subscribeWithSelector((s) => ({ upload: s.uploads[localId] }), ({ upload: nextUpload }) => {
...
_AttachmentManager.toLocalUploadAttachment = (fileLike) => {
	...
	if (isFileReference(fileLike) && fileLike.thumb_url) localAttachment.thumb_url = fileLike.thumb_url;
	if (isFileReference(fileLike) && fileLike.duration) localAttachment.duration = fileLike.duration;
	return localAttachment;
};
...
var DEFAULT_LINK_PREVIEW_MANAGER_CONFIG = {
	...
	findURLFn: (text) => find(text, "url", { defaultProtocol: "https" }).reduce((acc, link) => {
		try {
			...
			if (link.isLink && /^[a-zA-Z0-9-.]+\.[a-zA-Z]{2,}$/.test(url.hostname)) acc.push(link.href);
		} catch {}
		return acc;
	}, [])
};
...
var initState3 = ({ message }) => message ? { previews: message.attachments?.reduce((acc, attachment) => {
	...
	return acc;
}, /* @__PURE__ */ new Map()) ?? /* @__PURE__ */ new Map() } : { previews: /* @__PURE__ */ new Map() };
	...
	constructor({ composer, message }) {
		...
		this._findAndEnrichUrls = async (text) => {
			...
			const newLinkPreviews = urls.filter((url) => {
				...
				return !(existingPreviews.get(url) || existingPreviews.get(url + "/"));
			}).map((url) => ({
				og_scrape_url: url.trim(),
				status: "loading"
			}));
			if (!newLinkPreviews.length) return;
			this.state.partialNext({ previews: new Map([...keptPreviews, ...linkPreviewArrayToMap(newLinkPreviews)]) });
			await Promise.all(newLinkPreviews.map(async (linkPreview) => {
				try {
					const { duration, ...ogAttachment } = await this.client.enrichURL(linkPreview.og_scrape_url);
					if (this.shouldDiscardEnrichQueries) return;
					if (this.previews.has(linkPreview.og_scrape_url)) this.updatePreview(linkPreview.og_scrape_url, {
						status: "loaded",
						...ogAttachment
					});
				} catch (error) {
					if (this.previews.has(linkPreview.og_scrape_url)) this.updatePreview(linkPreview.og_scrape_url, { status: "failed" });
				}
				return linkPreview;
			}));
		};
		...
		*/
		this.clearPreviews = () => {
			...
			currentPreviews.forEach((preview, url) => {
				if (_LinkPreviewsManager.previewIsDismissed(preview)) newPreviews.set(url, preview);
			});
		...
		this.updatePreview = (url, preview) => {
			...
			const status = preview.status ?? this.previews.get(url)?.status ?? "pending";
			let finalPreview = preview;
			if (existingPreview) {
				const merged = mergeWithDiff(existingPreview, preview);
				if (!merged.diff || Object.keys(merged.diff).length === 0) return;
				finalPreview = merged.result;
			}
			this.state.partialNext({ previews: new Map(this.previews).set(url, {
				...finalPreview,
				og_scrape_url: url,
				status
			}) });
		...
		this.dismissPreview = (url) => {
			...
			if (preview) {
				...
				this.updatePreview(url, { status: "dismissed" });
			}
		};
		this.composer = composer;
		this.state = new StateStore(initState3({ message: this.enabled ? message : void 0 }));
...
_LinkPreviewsManager.previewIsLoading = (preview) => preview.status === "loading";
_LinkPreviewsManager.previewIsLoaded = (preview) => preview.status === "loaded";
_LinkPreviewsManager.previewIsDismissed = (preview) => preview.status === "dismissed";
_LinkPreviewsManager.previewIsFailed = (preview) => preview.status === "failed";
_LinkPreviewsManager.previewIsPending = (preview) => preview.status === "pending";
...
var initState4 = ({ message }) => ({ location: message?.shared_location ?? null });
var LocationComposer = class {
	constructor({ composer, message }) {
		...
		this.setData = (data) => {
			...
			this.state.partialNext({ location: {
				...data,
	...
	get validLocation() {
		...
		if (!!location?.created_by_device_id && location.message_id && location.latitude && location.longitude && (typeof durationMs === "undefined" || durationMs >= MIN_LIVE_LOCATION_SHARE_DURATION)) return {
			...location,
...
var applyCommandActivationEffect = (effect, composer) => {
	...
	if (effect.stateToRestore) snapshot.textComposer = {
...
var MessageComposerEffectHandlers = class {
	constructor(options) {
		...
		this.registerDefaultHandlers = () => {
			this.registerEffectHandler("command.activate", applyCommandActivationEffect);
			this.registerEffectHandler("command.clear", applyCommandClearEffect);
		};
		...
		this.applyEffect = (effect) => {
			this.handlers.get(effect.type)?.(effect, this.options.composer);
		};
		this.registerDefaultHandlers();
	}
};
...
var pollStateChangeValidators = {
	...
	max_votes_allowed: ({ data, value }) => {
		if (data.enforce_unique_vote && value) return { max_votes_allowed: "Enforce unique vote is enabled" };
		if (!value.match(/^[0-9]+$/) && value) return { max_votes_allowed: "Only numbers are allowed" };
		if (value?.length > 1 && !value.match(VALID_MAX_VOTES_VALUE_REGEX)) return { max_votes_allowed: "Type a number from 2 to 10" };
	...
	options: ({ value: options }) => {
		...
		options.forEach((option) => {
			if (seenOptions.has(option.text) && option.text.length) errors[option.id] = "Option already exists";
			else seenOptions.add(option.text);
		});
		return Object.keys(errors).length > 0 ? { options: errors } : { options: void 0 };
	}
};
var defaultPollFieldChangeEventValidators = { name: ({ currentError, value }) => value && currentError ? { name: void 0 } : { name: typeof currentError === "string" ? currentError : void 0 } };
var defaultPollFieldBlurEventValidators = {
	max_votes_allowed: ({ value }) => {
		if (value && !value.match(VALID_MAX_VOTES_VALUE_REGEX)) return { max_votes_allowed: "Type a number from 2 to 10" };
		return { max_votes_allowed: void 0 };
	},
	name: ({ value }) => {
	...
	options: (params) => {
		const errors = (pollStateChangeValidators.options?.(params))?.options ?? {};
		params.value.forEach((option, index) => {
			const isTheLastOption = index === params.value.length - 1;
			if (textFieldIsEmpty(option.text) && !isTheLastOption) errors[option.id] = "Option is empty";
		});
		return Object.keys(errors).length > 0 ? { options: errors } : { options: void 0 };
	}
};
...
var pollCompositionStateProcessors = {
	...
	max_votes_allowed: ({ value }) => ({ max_votes_allowed: clampMaxVotesAllowed(value) }),
	options: ({ value, data }) => {
		if (Array.isArray(value)) return { options: value.map((option) => ({
			id: option.id,
		...
		if (prevOptions.length < 100 && !newOptions.some((option) => !option.text.trim())) newOptions.push({
			id: generateUUIDv4(),
			text: ""
		});
...
var createPollComposerStateMiddleware = ({ processors: customProcessors, validators: customValidators } = {}) => {
	const universalHandler = ({ state, validators, processors }) => {
		...
		} else if (!processors) newData = targetFields;
		else newData = Object.entries(targetFields).reduce((acc, [key, value]) => {
	...
	return {
		...
		handlers: {
			handleFieldChange: ({ state, next, forward }) => {
			...
			handleFieldBlur: ({ state, next, forward }) => {
				if (!state.targetFields) return forward();
...
var createPollCompositionValidationMiddleware = (composer) => ({
	...
	handlers: { compose: ({ discard, forward }) => {
...
var ErrorFromResponse = class extends Error {
	...
	toJSON() {
		...
		const joinable = [];
		for (const [key, value] of extra) if (typeof value !== "undefined" && value !== null) joinable.push(`${key}: ${value}`);
		return {
...
var PollComposer = class {
	constructor({ composer }) {
		...
		*/
		...
		this.compositionMiddlewareExecutor = new PollComposerCompositionMiddlewareExecutor({ composer });
...
var createAttachmentsCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, discard, forward }) => {
		const { attachmentManager } = composer;
		...
		if (attachmentManager.uploadsInProgressCount > 0) {
			composer.client.notifications.addWarning({
				...
				options: { type: "validation:attachment:upload:in-progress" }
			});
			return discard();
		}
...
var createDraftAttachmentsCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, forward }) => {
		const { attachmentManager } = composer;
		if (!attachmentManager) return forward();
...
var createCompositionDataCleanupMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next }) => {
		const common = { type: !composer.editedMessage?.type || composer.editedMessage.type === "error" ? "regular" : composer.editedMessage.type };
...
function isWSFailure(err) {
	if (typeof err.isWSFailure === "boolean") return err.isWSFailure;
	try {
...
var BaseSearchSource = class extends BaseSearchSourceBase {
	...
	async executeQuery(newSearchString) {
		...
		try {
			...
			if (isAPIError(e) && !isErrorRetryable(e)) stateUpdate.hasNext = false;
		} finally {
...
var BasePaginator = class {
	...
	async executeQuery({ direction }) {
		...
		if (isFirstPage) this.state.next(this.getStateBeforeFirstQuery());
		else this.state.partialNext({ isLoading: true });
		const stateUpdate = {};
		try {
			...
			if (isFirstPage && (next || prev)) this._isCursorPagination = true;
...
var FilterBuilder = class {
	constructor(params) {
		...
		this.filterConfig = new StateStore(params?.initialFilterConfig ?? {});
	}
	...
	enableFilter(filterKey) {
		...
		if (config[filterKey]) this.filterConfig.partialNext({ [filterKey]: {
			...config[filterKey],
			enabled: true
		} });
	}
...
var ReminderPaginator = class extends BasePaginator {
	constructor(client, options) {
		super(options);
		this.query = async ({ direction }) => {
			const cursor = this.cursor?.[direction];
			const { reminders: items, next, prev } = await this.client.queryReminders({
				filter: this.filters,
				sort: this.sort,
				limit: this.pageSize,
				[direction]: cursor
...
var UserSearchSource = class extends BaseSearchSource {
	constructor(client, options, filterBuilderOptions = {}) {
		...
		this.filterBuilder = new FilterBuilder({
			initialFilterConfig: { $or: {
				enabled: true,
				generate: ({ searchQuery }) => searchQuery ? { $or: [{ id: { $autocomplete: searchQuery } }, { name: { $autocomplete: searchQuery } }] } : null
			} },
...
var ChannelSearchSource = class extends BaseSearchSource {
	...
	async query(searchQuery) {
		...
		return { items: await this.client.queryChannels(filters, sort, options) };
	}
	filterQueryResults(items) {
		return items;
	}
...
var MessageSearchSource = class extends BaseSearchSource {
	...
	async query(searchQuery) {
		...
		if (cids.length > 0) {
			...
			await this.client.queryChannels(channelQueryFilters, {
				last_message_at: -1,
...
var getTriggerCharWithToken = ({ trigger, text, isCommand = false, acceptTrailingSpaces = true }) => {
	const escapedTrigger = escapeCommandRegExp(trigger);
	const triggerNorWhitespace = `[^\\s${escapedTrigger}]*`;
	const match = text.match(new RegExp(isCommand ? `^[${escapedTrigger}]${triggerNorWhitespace}$` : acceptTrailingSpaces ? `(?!^|\\W)?[${escapedTrigger}]${triggerNorWhitespace}\\s?${triggerNorWhitespace}$` : `(?!^|\\W)?[${escapedTrigger}]${triggerNorWhitespace}$`, "g"));
	return match && match[match.length - 1].trim();
};
var insertItemWithTrigger = ({ insertText, selection, text, trigger }) => {
	...
	return {
		text: beforeCursor.slice(0, lastIndex) + insertText + afterCursor,
		selection: {
...
var replaceWordWithEntity = async ({ caretPosition, getEntityString, text }) => {
	const match = /([^\s]+)(\s*)$/.exec(text.slice(0, caretPosition));
...
var getTokenizedSuggestionDisplayName = ({ displayName, searchToken }) => ({ tokenizedDisplayName: {
	token: searchToken,
...
var CommandSearchSource = class extends BaseSearchSourceSync {
	...
	query(searchQuery) {
		const selectedCommands = (this.channel.getConfig()?.commands || []).filter((command) => !!(command.name && command.name.toLowerCase().indexOf(searchQuery.toLowerCase()) !== -1));
		selectedCommands.sort((a, b) => {
			let nameA = a.name?.toLowerCase();
			let nameB = b.name?.toLowerCase();
			if (nameA?.indexOf(searchQuery) === 0) nameA = `0${nameA}`;
			if (nameB?.indexOf(searchQuery) === 0) nameB = `0${nameB}`;
		...
		handlers: {
			onChange: ({ state, next, complete, forward }) => {
				...
				if (commandName) {
					...
					if (command && !composer?.isCommandDisabled(command)) return next({
						...state,
						command,
						suggestions: void 0
					});
				...
				if (triggerWithToken && triggerWithToken.length === finalOptions.minChars) searchSource.resetStateAndActivate();
				if (!triggerWithToken || triggerWithToken.length < finalOptions.minChars) {
					...
					if (hasStaleSuggestions) delete newState.suggestions;
			...
			onSuggestionItemSelect: ({ state, next, forward }) => {
				...
				const composer = options?.composer;
				if (composer && notifyCommandDisabled(composer, selectedSuggestion)) return forward();
...
var getDisabledRawCommand = (composer, searchSource, text) => {
	...
	if (rawCommand && composer.isCommandDisabled(rawCommand)) return rawCommand;
};
var createCompositionValidationMiddleware = (composer) => {
	...
	return {
		...
		handlers: { compose: async ({ state, discard, forward }) => {
			const { maxLengthOnSend } = composer.config.text ?? {};
			const inputText = state.message.text ?? "";
			const disabledRawCommand = getDisabledRawCommand(composer, commandSearchSource, inputText);
			...
			if (composer.compositionIsEmpty || hasExceededMaxLength) return await discard();
...
var createDraftCompositionValidationMiddleware = (composer) => ({
	...
	handlers: { compose: async ({ state, discard, forward }) => {
		...
		if (!(composer.lastChangeOriginIsLocal && hasData)) return await discard();
		return await forward();
	} }
});
var createLinkPreviewsCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, forward }) => {
		...
		if (!someLinkPreviewsLoading && linkPreviews.length > 0 || someLinkPreviewsDismissed) sendOptions.skip_enrich_url = true;
		return next({
			...state,
			message: {
				...state.message,
...
var createTextComposerCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, forward }) => {
		...
		const mentioned_users = Array.from(new Set(mentionedUsers.filter(({ id, name }) => text.includes(`@${id}`) || text.includes(`@${name}`))));
...
var createDraftTextComposerCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, forward }) => {
		...
		const mentioned_users = mentionedUsers.length ? Array.from(new Set(mentionedUsers.filter(({ id, name }) => inputText.includes(`@${id}`) || inputText.includes(`@${name}`)))) : void 0;
...
var createMessageComposerStateCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next }) => {
		const payload = {};
		if (composer.quotedMessage) payload.quoted_message_id = composer.quotedMessage.id;
		if (composer.pollId) payload.poll_id = composer.pollId;
		if (composer.showReplyInChannel) payload.show_in_channel = true;
		return next({
			...state,
			localMessage: {
				...state.localMessage,
...
var createUserDataInjectionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, next, forward }) => {
		if (!composer.client.user) return forward();
...
var createPollOnlyCompositionMiddleware = (composer) => ({
	...
	handlers: { compose: ({ state, complete, forward }) => {
...
var createActiveCommandGuardMiddleware = () => ({
	handlers: {
		onChange: ({ complete, forward, state }) => {
			if (state.command) return complete(state);
			return forward();
		},
...
var createCommandEffectsMiddleware = () => ({
	handlers: {
		...
		onSuggestionItemSelect: ({ state, next, forward }) => {
			const { selectedSuggestion } = state.change ?? {};
			if (!isCommandResponse(selectedSuggestion) || !state.command || state.command.name !== selectedSuggestion.name) return forward();
			return next({
				...state,
...
var createCommandStringExtractionMiddleware = () => ({
	handlers: {
		onChange: ({ complete, forward, state }) => {
			const { command } = state;
			if (!command?.name) return forward();
...
var calculateLevenshtein = (query, name) => {
	...
	const matrix = [];
	let i;
	for (i = 0; i <= name.length; i++) matrix[i] = [i];
	let j;
	for (j = 0; j <= query.length; j++) matrix[0][j] = j;
	for (i = 1; i <= name.length; i++) for (j = 1; j <= query.length; j++) if (name.charAt(i - 1) === query.charAt(j - 1)) matrix[i][j] = matrix[i - 1][j - 1];
...
var MentionsSearchSource = class extends BaseSearchSource {
	constructor(channel, options) {
		...
		this.getMembersAndWatchers = () => {
			const memberUsers = Object.values(this.channel.state.members ?? {}).map(({ user }) => user);
			const watcherUsers = Object.values(this.channel.state.watchers ?? {});
			const users = [...memberUsers, ...watcherUsers];
			const uniqueUsers = {};
			users.forEach((user) => {
				if (user && !uniqueUsers[user.id]) uniqueUsers[user.id] = user;
			});
		...
		this.searchMembersLocally = (searchQuery) => {
			...
			return this.getMembersAndWatchers().filter((user) => {
				...
				const maxDistance = 3;
				const lastDigits = textComposerText.slice(-4).includes("@");
				if (updatedName) {
					const levenshtein2 = calculateLevenshtein(updatedQuery, updatedName);
					if (updatedName.includes(updatedQuery) || levenshtein2 <= maxDistance && lastDigits) return true;
				}
		...
		this.prepareQueryMembersParams = (searchQuery) => {
			...
			let sort = [{ user_id: 1 }];
			if (!this.memberSort) sort = [{ user_id: 1 }];
			else if (Array.isArray(this.memberSort)) sort = this.memberSort[0];
			else if (Object.keys(this.memberSort).length === maxSortParamsCount) sort = this.memberSort;
			return {
		...
		this.queryMembers = async (searchQuery) => {
			...
			return (await this.channel.queryMembers(filters, sort, options)).members.map((member) => member.user);
		};
		this.filterMutes = (data) => {
			const { textComposerText } = this.config;
			if (!textComposerText) return [];
			const { mutedUsers } = this.client;
			if (textComposerText.includes("/unmute") && !mutedUsers.length) return [];
			if (!mutedUsers.length) return data;
			if (textComposerText.includes("/unmute")) return data.filter((suggestion) => mutedUsers.some((mute) => mute.target.id === suggestion.id));
		...
		if (transliterate) this.transliterate = transliterate;
	}
	get allMembersLoadedWithInitialChannelQuery() {
		return Object.keys(this.channel.state.members || {}).length < MAX_CHANNEL_MEMBER_COUNT_IN_CHANNEL_QUERY;
	}
	getStateBeforeFirstQuery(newSearchString) {
		const newState = super.getStateBeforeFirstQuery(newSearchString);
		const { items } = this.state.getLatestValue();
	...
	async query(searchQuery) {
		...
		if (this.config.mentionAllAppUsers) users = await this.queryUsers(searchQuery);
		else if (shouldSearchLocally) users = this.searchMembersLocally(searchQuery);
		else users = await this.queryMembers(searchQuery);
		return { items: users.map(this.toUserSuggestion) };
	}
	filterQueryResults(items) {
...
var createMentionsMiddleware = (channel, options) => {
	...
	if (options?.searchSource) {
		searchSource = options.searchSource;
		searchSource.resetState();
	} else searchSource = new MentionsSearchSource(channel);
	searchSource.activate();
	return {
...
var TextComposerMiddlewareExecutor = class extends MiddlewareExecutor {
	constructor({ composer }) {
		...
		this.use([
			...
			createCommandsMiddleware(composer.channel, { composer }),
			createCommandEffectsMiddleware()
		]);
	}
	async execute({ eventName, initialValue: initialState }) {
...
var initState5 = ({ composer, message }) => {
	if (!message) {
	...
	return {
		mentionedUsers: (message.mentioned_users ?? []).map((item) => typeof item === "string" ? { id: item } : item),
		text,
		selection: {
			start: text.length,
			end: text.length
...
var TextComposer = class {
	constructor({ composer, message }) {
		...
		this.setCommand = (command) => {
			...
			const stateToRestore = { command: null };
			this.commitState({
				...this.state.getLatestValue(),
				command,
				effects: [{
		...
		this.insertText = async ({ text, selection }) => {
			if (!this.enabled) return;
		...
		this.wrapSelection = ({ head = "", selection, tail = "" }) => {
			if (!this.enabled) return;
		...
		this.handleChange = async ({ text, selection }) => {
			if (!this.enabled) return;
			...
			if (this.config.publishTypingEvents && text) logChatPromiseExecution(this.channel.keystroke(this.composer.threadId ?? void 0), "start typing event");
		};
		this.handleSelect = async (target) => {
			...
			const output = await this.middlewareExecutor.execute({
				...
				initialValue: {
					...this.state.getLatestValue(),
					change: { selectedSuggestion: target }
				}
			});
			if (output?.status === "discard") return;
			this.commitState(output.state);
		...
		this.composer = composer;
		this.state = new StateStore(initState5({
			composer,
			message
		}));
...
var constructCustomDataObject = (threadData) => {
	...
	for (const key in threadData) {
		if (THREAD_RESPONSE_RESERVED_KEYS[key]) continue;
		const customKey = key;
...
var Thread = class extends WithSubscriptions {
	constructor({ client, threadData }) {
		...
		this.reload = async () => {
			if (this.state.getLatestValue().isLoading) return;
		...
		this.hydrateState = (thread) => {
			...
			if (thread.id !== this.id) throw new Error("Cannot hydrate thread's state using thread with different threadId");
			const { createdAt, custom, title, deletedAt, parentMessage, participants, read, replyCount, replies, updatedAt } = thread.state.getLatestValue();
		...
		this.subscribeThreadUpdated = () => this.client.on("thread.updated", (event) => {
			if (!event.thread || event.thread.parent_message_id !== this.id) return;
		...
		this.subscribeMessageDeleted = () => this.client.on("message.deleted", (event) => {
			...
			if (event.message.parent_id === this.id) if (event.hard_delete) this.deleteReplyLocally({ message: event.message });
			else this.upsertReplyLocally({ message: event.message });
			if (event.message.id === this.id) this.updateParentMessageLocally({ message: event.message });
		}).unsubscribe;
		this.subscribeMessageUpdated = () => {
			const unsubscribeFunctions = [
				...
				if (event.message) this.updateParentMessageOrReplyLocally(event.message);
			}).unsubscribe);
		...
		this.deleteReplyLocally = ({ message }) => {
			...
			if (replies[index]?.id !== message.id) return;
			const updatedReplies = [...replies];
			updatedReplies.splice(index, 1);
			this.state.partialNext({ replies: updatedReplies });
		};
		this.upsertReplyLocally = ({ message, timestampChanged = false }) => {
			if (message.parent_id !== this.id) throw new Error("Reply does not belong to this thread");
			const formattedMessage = formatMessage(message);
			if (message.status === "failed") this.failedRepliesMap.set(formattedMessage.id, formattedMessage);
			else if (this.failedRepliesMap.has(message.id)) this.failedRepliesMap.delete(message.id);
			this.state.next((current) => ({
				...current,
		...
		this.updateParentMessageLocally = ({ message }) => {
			if (message.id !== this.id) throw new Error("Message does not belong to this thread");
			this.state.next((current) => {
				const formattedMessage = formatMessage(message);
				return {
					...current,
		...
		this.updateParentMessageOrReplyLocally = (message) => {
			if (message.parent_id === this.id) this.upsertReplyLocally({ message });
			if (!message.parent_id && message.id === this.id) this.updateParentMessageLocally({ message });
		};
		this.markAsRead = async ({ force = false } = {}) => {
			if (this.ownUnreadCount === 0 && !force) return null;
			return await this.client.messageDeliveryReporter.markRead(this);
		};
		this.throttledMarkAsRead = throttle(() => this.markAsRead(), MARK_AS_READ_THROTTLE_TIMEOUT, { trailing: true });
		this.queryReplies = ({ limit = DEFAULT_PAGE_LIMIT, sort = DEFAULT_SORT, ...otherOptions } = {}) => this.channel.getReplies(this.id, {
			limit,
			...otherOptions
		}, sort);
		...
		this.loadPage = async (count) => {
			...
			this.state.partialNext({ pagination: {
				...pagination,
				[loadingKey]: true
			} });
			try {
		...
		const channel = client.channel(threadData.channel.type, threadData.channel.id, { name: threadData.channel.name });
...
var repliesPaginationFromInitialThread = (thread) => {
	return {
		nextCursor: null,
		prevCursor: thread.latest_replies.length === thread.reply_count ? null : thread.latest_replies.at(0)?.id ?? null,
		isLoadingNext: false,
		isLoadingPrev: false
	};
};
...
var initEditingAuditState = (composition) => {
	...
	if (compositionIsDraftResponse(composition)) stateUpdate = draftUpdate = new Date(composition.created_at).getTime();
	else if (composition && isLocalMessage(composition)) stateUpdate = new Date(composition.updated_at).getTime();
	return { lastChange: {
		draftUpdate,
		stateUpdate
	} };
...
var _MessageComposer = class _MessageComposer extends WithSubscriptions {
	constructor({ composition, config, compositionContext, client }) {
		super();
		this.snapshots = [];
		this.setEditedMessage = (editedMessage) => {
			this.state.partialNext({ editedMessage: editedMessage ?? null });
			if (editedMessage) this.textComposer.clearCommand();
		};
		this.getCommandDisabledReason = (command) => {
			if (this.editedMessage) return "editing";
			if (this.quotedMessage && (command.set === "moderation_set" || command.name === "moderation_set")) return "quoted_message";
		};
		...
		this.initStateFromChannelResponse = (channelApiResponse) => {
			if (this.channel.cid !== channelApiResponse.channel.cid) return;
			if (channelApiResponse.draft) this.initState({ composition: channelApiResponse.draft });
		...
		this.subscribeMessageUpdated = () => {
			const unsubscribeFunctions = [
				...
				if (!event.message) return;
				if (event.message.id === this.id) this.initState({ composition: event.message });
				if (this.quotedMessage?.id && event.message.id === this.quotedMessage.id) this.setQuotedMessage(formatMessage(event.message));
			}).unsubscribe);
			return () => unsubscribeFunctions.forEach((unsubscribe) => unsubscribe());
		};
		this.subscribeMessageComposerSetupStateChange = () => {
			let tearDown = null;
			const unsubscribe = this.client._messageComposerSetupState.subscribeWithSelector(({ setupFunction: setup }) => ({ setup }), ({ setup }) => {
				tearDown?.();
		...
		this.subscribeMessageDeleted = () => this.client.on("message.deleted", (event) => {
			if (!event.message) return;
			if (event.message.id === this.id) this.clear();
			else if (this.quotedMessage && event.message.id === this.quotedMessage.id) this.setQuotedMessage(null);
		}).unsubscribe;
		...
		this.subscribeDraftDeleted = () => this.client.on("draft.deleted", (event) => {
			...
			if (this.compositionIsEmpty) return;
			this.clear();
		}).unsubscribe;
		this.subscribeTextComposerStateChanged = () => this.textComposer.state.subscribeWithSelector(({ text }) => [text], ([currentText], previousSelection) => {
			...
			if (!currentText) this.linkPreviewsManager.clearPreviews();
			else this.linkPreviewsManager.findAndEnrichUrls(currentText);
		});
		...
		this.subscribeCustomDataManagerStateChanged = () => this.customDataManager.state.subscribe((nextValue, previousValue) => {
			if (typeof previousValue !== "undefined" && !this.customDataManager.isMessageDataEqual(nextValue, previousValue)) this.logStateUpdateTimestamp();
		});
		this.subscribeMessageComposerStateChanged = () => this.state.subscribe((_, previousValue) => {
			if (typeof previousValue === "undefined") return;
			this.logStateUpdateTimestamp();
			if (this.compositionIsEmpty) this.deleteDraft();
		});
		this.subscribeMessageComposerConfigStateChanged = () => {
			...
			const unsubscribe = this.configState.subscribeWithSelector((currentValue) => ({
				...
				if (this.textComposer.text === "" && textDefaultValue) this.textComposer.insertText({
				...
				if (draftsEnabled && !draftUnsubscribeFunction) draftUnsubscribeFunction = this.registerDraftEventSubscriptions();
		...
		this.setQuotedMessage = (quotedMessage) => {
			...
			if (quotedMessage && activeCommand && this.isCommandDisabled(activeCommand)) this.textComposer.clearCommand();
		};
		...
		this.getDraft = async () => {
			...
			if (draftFromOfflineDB) this.initState({ composition: draftFromOfflineDB });
			try {
				const { draft } = await this.channel.getDraft({ parent_id: this.threadId ?? void 0 });
				if (!draft) return;
				this.client.offlineDb?.executeQuerySafely((db) => db.upsertDraft({ draft }), { method: "upsertDraft" });
				this.initState({ composition: draft });
			} catch (error) {
		...
		if (compositionContext instanceof Channel) this.channel = compositionContext;
		else if (compositionContext instanceof Thread) this.channel = compositionContext.channel;
		else if (compositionContext.cid) {
			const [type, id] = compositionContext.cid.split(":");
			this.channel = client.channel(type, id);
		} else throw new Error("MessageComposer requires composition context pointing to channel (channel or context.cid)");
		const mergeChannelConfigCustomizer = (originalVal, channelConfigVal, key) => typeof originalVal === "object" ? void 0 : originalVal === false && key === "enabled" ? false : [
			"string",
			"number",
			"bigint",
			"boolean",
			"symbol"
		].includes(typeof channelConfigVal) ? channelConfigVal : originalVal;
		this.configState = new StateStore(mergeWith(mergeWith(DEFAULT_COMPOSER_CONFIG, config ?? {}), { location: { enabled: this.channel.getConfig()?.shared_locations } }, mergeChannelConfigCustomizer));
		let message = void 0;
		if (compositionIsDraftResponse(composition)) message = composition.message;
		else if (composition) message = formatMessage(composition);
		...
		this.compositionMiddlewareExecutor = new MessageComposerMiddlewareExecutor({ composer: this });
		this.draftCompositionMiddlewareExecutor = new MessageDraftComposerMiddlewareExecutor({ composer: this });
		this.effectHandlers = new MessageComposerEffectHandlers({ composer: this });
	}
	static evaluateContextType(compositionContext) {
		if (compositionContext instanceof Channel) return "channel";
		if (compositionContext instanceof Thread) return "thread";
		if (typeof compositionContext.legacyThreadId === "string") return "legacy_thread";
		return "message";
	}
	...
	get threadId() {
		if (this.compositionContext instanceof Channel) return null;
		if (this.compositionContext instanceof Thread) return this.compositionContext.id;
		if (typeof this.compositionContext.legacyThreadId === "string") return this.compositionContext.legacyThreadId;
		if (typeof this.compositionContext.parent_id === "string") return this.compositionContext.parent_id;
		return null;
	}
	get client() {
	...
	get lastChangeOriginIsLocal() {
		const composingMessageFromScratch = this.lastChange.draftUpdate === null && !this.editedMessage;
	...
	logStateUpdateTimestamp() {
		this.editingAuditState.partialNext({ lastChange: {
			...this.lastChange,
...
var MessageDeliveryReporter = class _MessageDeliveryReporter {
	constructor({ client }) {
		...
		*/
		this.getNextDeliveryReportCandidate = (collection) => {
			...
			if (!latestMessage || lastReadAt >= latestMessage.created_at) return {
				key,
				id: null
			};
			if (!latestMessage || (lastDeliveredAt ?? 0) >= latestMessage.created_at) return {
				key,
				id: null
			};
			return {
		...
		*/
		this.announceDelivery = (options) => {
			...
			const postFlightReconcile = ({ preventSchedulingRetry } = {}) => {
				this.markDeliveredRequestPromise = null;
				for (const [k, v] of this.nextDeliveryReportCandidates.entries()) this.deliveryReportCandidates.set(k, v);
				this.nextDeliveryReportCandidates = /* @__PURE__ */ new Map();
			...
			const handleError = (error) => {
				...
				for (const [k, v] of this.deliveryReportCandidates.entries()) newDeliveryReportCandidates.set(k, v);
				this.deliveryReportCandidates = newDeliveryReportCandidates;
				if (isAPIError(error) && isErrorRetryable(error) || error.status >= 500) {
					this.increaseBackOff();
					postFlightReconcile();
				} else postFlightReconcile({ preventSchedulingRetry: true });
			};
		...
		*/
		this.markRead = async (collection, options) => {
			let result = null;
			if (isChannel(collection)) result = await collection.markAsReadRequest(options);
			else if (isThread(collection)) result = await collection.channel.markAsReadRequest({
				...options,
	...
	*/
	...
	confirmationsFromDeliveryReportCandidates() {
		...
		this.deliveryReportCandidates = new Map(entries.slice(MAX_DELIVERED_MESSAGE_COUNT_IN_PAYLOAD));
		return {
			latest_delivered_messages: this.confirmationsFrom(sendBuffer),
			sendBuffer
		};
	}
	/**
...
var MessageReceiptsTracker = class {
	...
	ingestInitial(responses) {
		...
		for (const r of responses) {
			...
			if (compareRefsAsc(lastDeliveredRef, lastReadRef) < 0) lastDeliveredRef = lastReadRef;
			const userProgress = {
				user: r.user,
				lastReadRef,
				lastDeliveredRef
			...
			this.readSorted.splice(findIndex(this.readSorted, lastReadRef, (up) => up.lastReadRef), 0, userProgress);
			this.deliveredSorted.splice(findIndex(this.deliveredSorted, lastDeliveredRef, (up) => up.lastDeliveredRef), 0, userProgress);
		}
	}
	...
	onMessageDelivered({ user, deliveredAt, lastDeliveredMessageId }) {
		...
		removeByOldKey(this.deliveredSorted, userProgress, userProgress.lastDeliveredRef, (x) => x.lastDeliveredRef);
	...
	onMessageRead({ user, readAt, lastReadMessageId }) {
		...
		removeByOldKey(this.readSorted, userProgress, userProgress.lastReadRef, (x) => x.lastReadRef);
	...
	*/
	onNotificationMarkUnread({ user, lastReadAt, lastReadMessageId }) {
		const userProgress = this.ensureUser(user);
		const newReadRef = lastReadAt ? {
			timestampMs: new Date(lastReadAt).getTime(),
			msgId: lastReadMessageId ?? ""
		} : { ...MIN_REF };
		if (compareRefsAsc(newReadRef, userProgress.lastReadRef) === 0 && newReadRef.msgId === userProgress.lastReadRef.msgId) return;
		removeByOldKey(this.readSorted, userProgress, userProgress.lastReadRef, (x) => x.lastReadRef);
	...
	deliveredForMessage(msgRef) {
		const pos = findIndex(this.deliveredSorted, msgRef, ({ lastDeliveredRef }) => lastDeliveredRef);
	...
	deliveredNotReadForMessage(msgRef) {
		...
		for (let i = pos; i < this.deliveredSorted.length; i++) {
			...
			if (compareRefsAsc(userProgress.lastReadRef, msgRef) < 0) usersDeliveredNotRead.push(userProgress.user);
		}
	...
	groupUsersByLastReadMessage() {
		return Array.from(this.byUser.values()).reduce((acc, userProgress) => {
...
var Channel = class {
	...
	constructor(client, type, id, data) {
		...
		*/
		...
		this._callChannelListeners = (event) => {
			const channel = this;
			const listeners = [];
			if (channel.listeners.all) listeners.push(...channel.listeners.all);
			if (channel.listeners[event.type]) listeners.push(...channel.listeners[event.type]);
			for (const listener of listeners) if (typeof listener !== "string") listener(event);
		};
		/**
		...
		*/
		this._channelURL = () => {
			if (!this.id) throw new Error("channel id is not defined");
			return `${this.getClient().baseURL}/channels/${encodeURIComponent(this.type)}/${encodeURIComponent(this.id)}`;
		};
		const validTypeRe = /^[\w_-]+$/;
		const validIDRe = /^[\w!_-]+$/;
		if (!validTypeRe.test(type)) throw new Error(`Invalid chat type ${type}, letters, numbers and "_-" are allowed`);
		if (typeof id === "string" && !validIDRe.test(id)) throw new Error(`Invalid chat id ${id}, letters, numbers and "!-_" are allowed`);
		this._client = client;
		this.type = type;
		this.id = id;
		this.data = data;
		...
		this.messageReceiptsTracker = new MessageReceiptsTracker({ locateMessage: (timestampMs) => {
	...
	*/
	getClient() {
		if (this.disconnected === true) throw Error(`You can't use a channel after client.disconnect() was called`);
		return this._client;
	}
	/**
	...
	async sendMessage(message, options) {
		try {
			...
			if (offlineDb) {
				...
				if (messageId) return await offlineDb.queueTask({ task: {
					channelId: this.id,
	...
	*/
	sendFile(uri, name, contentType, user, axiosRequestConfig) {
		return this.getClient().sendFile(`${this._channelURL()}/file`, uri, name, contentType, user, axiosRequestConfig);
	}
	/**
	...
	*/
	sendImage(uri, name, contentType, user, axiosRequestConfig) {
		return this.getClient().sendFile(`${this._channelURL()}/image`, uri, name, contentType, user, axiosRequestConfig);
	}
	deleteFile(url) {
	...
	*/
	async sendEvent(event) {
		...
		return await this.getClient().post(this._channelURL() + "/event", { event });
	}
	/**
	...
	*/
	async search(query, options = {}) {
		if (options.offset && options.next) throw Error(`Cannot specify offset with next`);
		const payload = {
		...
		else if (typeof query === "object") payload.message_filter_conditions = query;
		else throw Error(`Invalid type ${typeof query} for query parameter`);
		await this.getClient().wsPromise;
		return await this.getClient().get(this.getClient().baseURL + "/search", { payload });
	}
	/**
	...
	*/
	async queryMembers(filterConditions, sort = [], options = {}) {
		...
		else if (this.data?.members && Array.isArray(this.data.members)) members = this.data.members;
		return await this.getClient().get(this.getClient().baseURL + "/members", { payload: {
			type,
			id,
			members,
	...
	*/
	async partialUpdateMember(user_id, updates) {
		if (!user_id) throw Error("Please specify the user id");
	...
	*/
	async sendReaction(messageID, reaction, options) {
		if (!messageID) throw Error(`Message id is missing`);
		if (!reaction || Object.keys(reaction).length === 0) throw Error(`Reaction object is missing`);
		try {
			const offlineDb = this.getClient().offlineDb;
			if (offlineDb) return await offlineDb.queueTask({ task: {
				channelId: this.id,
				channelType: this.type,
				messageId: messageID,
				payload: [
	...
	*/
	...
	async deleteReaction(messageID, reactionType, user_id) {
		this._checkInitialized();
		if (!reactionType || !messageID) throw Error("Deleting a reaction requires specifying both the message and reaction type");
		try {
			const offlineDb = this.getClient().offlineDb;
			if (offlineDb) {
				...
				if (message) await offlineDb.deleteReaction({
					message,
					reaction
				});
				return await offlineDb.queueTask({ task: {
					channelId: this.id,
					channelType: this.type,
					messageId: messageID,
	...
	*/
	async _deleteReaction(messageID, reactionType, user_id) {
		...
		if (user_id) return await this.getClient().delete(url, { user_id });
	...
	*/
	async updatePartial(update) {
		...
		if (areCapabilitiesChanged) this.getClient().dispatchEvent({
	...
	*/
	async disableSlowMode() {
		const data = await this.getClient().post(this._channelURL(), { cooldown: 0 });
		this.data = data.channel;
		return data;
	}
	async sendSharedLocation(location, userId) {
		...
		if (location.end_at) this.getClient().dispatchEvent({
	...
	*/
	async delete(options = {}) {
		return await this.getClient().delete(this._channelURL(), { ...options });
	}
	/**
	...
	*/
	async archive(opts = {}) {
		...
		if (!uid) throw Error("A user_id is required for archiving a channel");
		return (await this.partialUpdateMember(uid, { set: { archived: true } })).channel_member;
	}
	/**
	...
	*/
	async unarchive(opts = {}) {
		const cli = this.getClient();
		const uid = opts.user_id || cli.userID;
		if (!uid) throw Error("A user_id is required for unarchiving a channel");
		return (await this.partialUpdateMember(uid, { set: { archived: false } })).channel_member;
	}
	/**
	...
	*/
	async pin(opts = {}) {
		const cli = this.getClient();
		const uid = opts.user_id || cli.userID;
		if (!uid) throw new Error("A user_id is required for pinning a channel");
		return (await this.partialUpdateMember(uid, { set: { pinned: true } })).channel_member;
	}
	/**
	...
	*/
	async unpin(opts = {}) {
		const cli = this.getClient();
		const uid = opts.user_id || cli.userID;
		if (!uid) throw new Error("A user_id is required for unpinning a channel");
		return (await this.partialUpdateMember(uid, { set: { pinned: false } })).channel_member;
	}
	/**
	...
	*/
	async keystroke(parent_id, options) {
		if (!this._isTypingIndicatorsEnabled()) return;
		const now = /* @__PURE__ */ new Date();
	...
	*/
	async clearAIIndicator() {
		await this.sendEvent({ type: "ai_indicator.clear" });
	}
	/**
	...
	*/
	async stopAIResponse() {
		await this.sendEvent({ type: "ai_indicator.stop" });
	}
	/**
	...
	*/
	...
	_isTypingIndicatorsEnabled() {
		if (!this.getConfig()?.typing_events || !this.getClient().wsConnection?.isHealthy) return false;
	...
	*/
	async markAsReadRequest(data = {}) {
		this._checkInitialized();
		if (!this.getConfig()?.read_events && !this.getClient()._isUsingServerAuth()) return null;
		return await this.getClient().post(this._channelURL() + "/read", { ...data });
	}
	/**
	...
	*/
	async markUnread(data) {
		this._checkInitialized();
		if (!this.getConfig()?.read_events && !this.getClient()._isUsingServerAuth()) return Promise.resolve(null);
		return await this.getClient().post(this._channelURL() + "/unread", { ...data });
	}
	/**
	...
	*/
	clean() {
		if (this.lastKeyStroke) {
			if ((/* @__PURE__ */ new Date()).getTime() - this.lastKeyStroke.getTime() > 1e3 && this.isTyping) logChatPromiseExecution(this.stopTyping(), "stop typing event");
		}
		this.state.clean();
	}
	/**
	...
	*/
	async watch(options) {
		...
		await this.getClient().wsPromise;
		if (!this.getClient()._hasConnectionID()) defaultOptions.watch = false;
		const combined = {
			...defaultOptions,
			...options
		};
	...
	*/
	async getReplies(parent_id, options, sort) {
		...
		if (data.messages) this.state.addMessagesSorted(data.messages);
		return data;
	}
	/**
	...
	*/
	async getPinnedMessages(options, sort = []) {
		return await this.getClient().get(this._channelURL() + "/pinned_messages", { payload: {
			...options,
			sort: normalizeQuerySort(sort)
		} });
	}
	...
	*/
	getReactions(message_id, options) {
		return this.getClient().get(this.getClient().baseURL + `/messages/${encodeURIComponent(message_id)}/reactions`, { ...options });
	}
	/**
	...
	*/
	getMessagesById(messageIds) {
		return this.getClient().get(this._channelURL() + "/messages", { ids: messageIds.join(",") });
	}
	/**
	...
	*/
	lastRead() {
		...
		if (userID) return this.state.read[userID] ? this.state.read[userID].last_read : null;
	}
	_countMessageAsUnread(message) {
		...
		if (Array.isArray(this.data?.own_capabilities) && !this.data?.own_capabilities.includes("read-events")) return false;
	...
	*/
	countUnread(lastRead) {
		...
		for (let i = 0; i < this.state.latestMessages.length; i += 1) {
			...
			if (message.created_at > lastRead && this._countMessageAsUnread(message)) count++;
		}
		return count;
	}
	/**
	...
	*/
	countUnreadMentions() {
		...
		for (let i = 0; i < this.state.latestMessages.length; i += 1) {
			const message = this.state.latestMessages[i];
			if (this._countMessageAsUnread(message) && (!lastRead || message.created_at > lastRead) && message.mentioned_users?.some((user) => user.id === userID)) count++;
		}
		return count;
	}
	/**
	...
	*/
	async query(options = {}, messageSetToAddToIfDoesNotExist = "current") {
		...
		if (this.getClient()._isUsingServerAuth() && typeof createdById !== "string") this.getClient().logger("warn", "Either `created_by` (with `id` property) or `created_by_id` are missing from both `Channel._data` and `options` parameter");
		let queryURL = `${this.getClient().baseURL}/channels/${encodeURIComponent(this.type)}`;
		if (this.id) queryURL += `/${encodeURIComponent(this.id)}`;
		...
		if (!this.id) {
			...
			if (tempChannelCid && tempChannelCid in this.getClient().activeChannels) delete this.getClient().activeChannels[tempChannelCid];
			if (!(this.cid in this.getClient().activeChannels) && this.getClient()._cacheEnabled()) this.getClient().activeChannels[this.cid] = this;
		}
		this.getClient()._addChannelConfig(state.channel);
		if (typeof state.channel.config?.shared_locations !== "undefined") this.messageComposer.updateConfig({ location: { enabled: state.channel.config.shared_locations } });
		const { messageSet, filteredMessageIds } = this._initializeState(state, messageSetToAddToIfDoesNotExist);
		messageSet.pagination = {
			...
			...messageSetPagination({
				...
				filteredReturnedPage: state.messages.filter((m) => !filteredMessageIds.includes(m.id)),
	...
	*/
	async show(userId = null) {
		this._checkInitialized();
		return await this.getClient().post(`${this._channelURL()}/show`, { user_id: userId });
	}
	/**
	...
	*/
	async _createDraft(message) {
		return await this.getClient().post(this._channelURL() + "/draft", { message });
	}
	/**
	...
	*/
	async _deleteDraft({ parent_id } = {}) {
		return await this.getClient().delete(this._channelURL() + "/draft", { parent_id });
	}
	/**
	...
	*/
	async getDraft({ parent_id } = {}) {
		return await this.getClient().get(this._channelURL() + "/draft", { parent_id });
	}
	...
	_handleChannelEvent(event) {
		...
		switch (event.type) {
			case "typing.start":
				if (event.user?.id) channelState.typing[event.user.id] = event;
				break;
			case "typing.stop":
				if (event.user?.id) delete channelState.typing[event.user.id];
				break;
			case "message.read":
			...
			case "message.delivered":
				if (event.user?.id && event.created_at) {
					...
					const client = this.getClient();
					if (event.user?.id === client.user?.id) client.syncDeliveredCandidates([this]);
				}
				break;
			case "user.watching.start":
			case "user.updated":
				if (event.user?.id) channelState.watchers[event.user.id] = event.user;
				break;
			case "user.watching.stop":
				if (event.user?.id) delete channelState.watchers[event.user.id];
				break;
			case "message.deleted":
				if (event.message) {
					...
					if (event.message.pinned) channelState.removePinnedMessage(event.message);
				}
				break;
			case "user.messages.deleted":
				if (event.user) this.state.deleteUserMessages(event.user, !!event.hard_delete, new Date(event.created_at ?? Date.now()));
				break;
			case "message.new":
				if (event.message) {
					const client = this.getClient();
					...
					if (this.state.isUpToDate || isThreadMessage) channelState.addMessageSorted(event.message, ownMessage);
					if (event.message.pinned) channelState.addPinnedMessage(event.message);
					const preventUnreadCountUpdate = ownMessage || isThreadMessage;
					if (ownMessage) this.cooldownTimer.refresh();
					if (preventUnreadCountUpdate) break;
					if (event.user?.id) for (const userId in channelState.read) if (userId === event.user.id) channelState.read[event.user.id] = {
						last_read: new Date(event.created_at),
						user: event.user,
						unread_messages: 0,
					...
					else channelState.read[userId].unread_messages += 1;
					if (this._countMessageAsUnread(event.message)) channelState.unreadCount = channelState.unreadCount + 1;
					client.syncDeliveredCandidates([this]);
				}
				break;
			...
			case "message.undeleted":
				if (event.message) {
					...
					if (event.message.pinned) channelState.addPinnedMessage(event.message);
					else channelState.removePinnedMessage(event.message);
				}
				break;
			...
			case "member.added":
			case "member.updated": {
				const memberCopy = { ...event.member };
				if (memberCopy.pinned_at === null) delete memberCopy.pinned_at;
				if (memberCopy.archived_at === null) delete memberCopy.archived_at;
				if (memberCopy?.user) {
					channelState.members = {
						...channelState.members,
						[memberCopy.user.id]: memberCopy
					};
					if (channel.data?.member_count && event.type === "member.added") channel.data.member_count += 1;
				}
				const currentUserId = this.getClient().userID;
				if (typeof currentUserId === "string" && typeof memberCopy?.user?.id === "string" && memberCopy.user.id === currentUserId) channelState.membership = memberCopy;
				break;
			}
			case "member.removed":
				if (event.user?.id) {
					const newMembers = { ...channelState.members };
					delete newMembers[event.user.id];
					channelState.members = newMembers;
					if (channel.data?.member_count) channel.data.member_count = Math.max(channel.data.member_count - 1, 0);
				}
				break;
			case "notification.mark_unread": {
				if (!(event.user?.id === this.getClient().user?.id) || !event.user) break;
			...
			case "channel.updated":
				if (event.channel) {
					if (event.channel?.frozen !== void 0 && event.channel.frozen !== channel.data?.frozen) this.query({
						state: false,
						messages: { limit: 0 },
						watchers: { limit: 0 }
					});
			...
			case "channel.hidden":
				...
				if (event.clear_history) channelState.clearMessages();
				break;
		...
		if (event.watcher_count !== void 0) channel.state.watcher_count = event.watcher_count;
	}
	_checkInitialized() {
		if (!this.initialized && !this.offlineMode && !this.getClient()._isUsingServerAuth()) throw Error(`Channel ${this.cid} hasn't been initialized yet. Make sure to call .watch() and wait for it to resolve`);
	}
	_initializeState(state, messageSetToAddToIfDoesNotExist = "latest") {
		const { state: clientState, user, userID } = this.getClient();
		if (state.members) {
			this._hydrateMembers({ members: state.members });
			for (const member of state.members) if (member.user) clientState.updateUserReference(member.user, this.cid);
		}
		this.state.membership = state.membership || {};
		const messages = state.messages || [];
		if (!this.state.messages) this.state.initMessages();
		const { messageSet, filteredMessageIds } = this.state.addMessagesSorted(messages, false, true, true, messageSetToAddToIfDoesNotExist);
		if (!this.state.pinnedMessages) this.state.pinnedMessages = [];
		this.state.addPinnedMessages(state.pinned_messages || []);
		if (state.pending_messages) this.state.pending_messages = state.pending_messages;
		if (state.watcher_count !== void 0) this.state.watcher_count = state.watcher_count;
		if (state.watchers) {
			for (const watcher of state.watchers) if (watcher) {
		...
		if (userID != null) {
			...
			if (user) this.state.read[user.id] = {
				user,
				last_read,
				unread_messages: 0
			};
		...
		if (state.read) {
			for (const read of state.read) {
				...
				if (read.user.id === user?.id) this.state.unreadCount = this.state.read[read.user.id].unread_messages;
			}
	...
	_extendEventWithOwnReactions(event) {
		...
		if (message) event.message.own_reactions = message.own_reactions;
	}
	_hydrateMembers({ members, overrideCurrentState = true }) {
		const newMembersById = members.reduce((membersById, member) => {
			if (member.user) membersById[member.user.id] = member;
			return membersById;
		}, {});
		if (overrideCurrentState) this.state.members = newMembersById;
		else if (!overrideCurrentState && members.length) this.state.members = {
			...this.state.members,
			...newMembersById
		};
	}
...
var ClientState = class {
	...
	updateUsers(users) {
		for (const user of users) this.updateUser(user);
	}
	updateUser(user) {
		if (user != null && this.client._cacheEnabled()) this.users[user.id] = user;
	}
	updateUserReference(user, channelID) {
		if (user == null || !this.client._cacheEnabled()) return;
		this.updateUser(user);
		if (!this.userChannelReferences[user.id]) this.userChannelReferences[user.id] = {};
		this.userChannelReferences[user.id][channelID] = true;
	}
	deleteAllChannelReference(channelID) {
		for (const userID in this.userChannelReferences) delete this.userChannelReferences[userID][channelID];
	}
};
...
var StableWSConnection = class {
	constructor({ client }) {
		...
		*/
		this.onlineStatusChanged = (event) => {
			if (event.type === "offline") {
				...
				if (!this.isHealthy) this._reconnect({ interval: 10 });
			}
		};
		this.onopen = (wsID) => {
		...
		this.onmessage = (wsID, event) => {
			...
			if (data.type === "health.check") this.scheduleNextPing();
		...
		this.onclose = (wsID, event) => {
			...
			if (event.code === chatCodes.WS_CLOSED_SUCCESS) {
				const error = /* @__PURE__ */ new Error(`WS connection reject with error ${event.reason}`);
		...
		*/
		this._errorFromWSEvent = (event, isWSFailure2 = true) => {
			...
			const error = /* @__PURE__ */ new Error(`WS failed with code ${code} and reason - ${message}`);
			error.code = code;
		...
		*/
		this.scheduleNextPing = () => {
			if (this.healthCheckTimeoutRef) clearTimeout(this.healthCheckTimeoutRef);
		...
		*/
		this.scheduleConnectionCheck = () => {
			if (this.connectionCheckTimeoutRef) clearTimeout(this.connectionCheckTimeoutRef);
	...
	*/
	async connect(timeout = 15e3) {
		if (this.isConnecting) throw Error(`You've called connect twice, can only attempt 1 connection at the time`);
		this.isDisconnected = false;
		try {
			...
			} else if (!e.isWSFailure) throw new Error(JSON.stringify({
				code: e.code,
	...
	*/
	_waitForHealthy(timeout = 15e3) {
		return Promise.race([(async () => {
			const interval = 50;
			for (let i = 0; i <= timeout; i += interval) try {
				return await this.connectionOpen;
			} catch (error) {
				if (i === timeout) throw new Error(JSON.stringify({
					code: error.code,
	...
	*/
	disconnect(timeout) {
		...
		if (this.healthCheckTimeoutRef) clearInterval(this.healthCheckTimeoutRef);
		if (this.connectionCheckTimeoutRef) clearInterval(this.connectionCheckTimeoutRef);
	...
	*/
	async _connect() {
		...
		try {
			...
			this.ws = new browser_default(wsURL);
	...
	*/
	async _reconnect(options = {}) {
		...
		if (!interval) interval = retryInterval(this.consecutiveFailures);
		await sleep(interval);
		if (this.isConnecting || this.isHealthy) {
		...
		if (options.refreshToken) await this.client.tokenManager.loadToken();
		try {
			await this._connect();
...
var UploadManager = class {
	constructor(client) {
		...
		*/
		this.reset = () => {
			for (const { abortController } of this.inFlightUploads.values()) abortController.abort();
		...
		*/
		this.upload = ({ id, channelCid, file }) => {
			...
			(async () => {
				...
				try {
					...
					const response = await attachmentManager.doUploadRequest(file, uploadRequestOptions);
		...
		this.upsertUpload = (record) => {
			this.state.partialNext({ uploads: upsertById(this.uploads, record) });
		};
	...
	resolveAttachmentManager(channelCid) {
		...
		if (colon <= 0 || colon === channelCid.length - 1) throw new Error(`Invalid channelCid: ${channelCid}`);
...
function JWTUserToken(apiSecret, userId, extraData = {}, jwtOptions = {}) {
	...
	if (import_jsonwebtoken.default == null || import_jsonwebtoken.default.sign == null) throw Error(`Unable to find jwt crypto, if you are getting this error is probably because you are trying to generate tokens on browser or React Native (or other environment where crypto functions are not available). Please Note: token should only be generated server-side.`);
	const opts = Object.assign({
		algorithm: "HS256",
		noTimestamp: true
	}, jwtOptions);
	if (payload.iat) opts.noTimestamp = false;
...
function gunzipPayload(rawBody) {
	...
	if (body.length >= 2 && body.subarray(0, 2).equals(GZIP_MAGIC)) try {
...
function decodeSqsPayload(body) {
	if (!/^[A-Za-z0-9+/]*={0,2}$/.test(body) || body.length % 4 !== 0) throw new InvalidWebhookError(InvalidWebhookErrorMessages.invalidBase64);
	const decoded = Buffer.from(body, "base64");
	if (decoded.toString("base64").length !== body.length) throw new InvalidWebhookError(InvalidWebhookErrorMessages.invalidBase64);
	return gunzipPayload(decoded);
}
function decodeSnsPayload(notificationBody) {
	return decodeSqsPayload(extractSnsMessage(notificationBody) ?? notificationBody);
}
function extractSnsMessage(notificationBody) {
	...
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed) || typeof parsed.Message !== "string") return null;
...
function verifyAndParse(payload, signature, secret) {
	if (!verifySignature(payload, signature, secret)) throw new InvalidWebhookError(InvalidWebhookErrorMessages.signatureMismatch);
	...
	constructor(secret) {
		...
		*/
		...
		this.validateToken = (tokenOrProvider, user) => {
			...
			if (!this.secret && !tokenOrProvider) throw new Error("User token can not be empty");
			if (tokenOrProvider && typeof tokenOrProvider !== "string" && !isFunction(tokenOrProvider)) throw new Error("user token should either be a string or a function");
			if (typeof tokenOrProvider === "string") {
				if (user.anon && tokenOrProvider === "") return;
				const tokenUserId = UserFromToken(tokenOrProvider);
				if (tokenOrProvider != null && (tokenUserId == null || tokenUserId === "" || tokenUserId !== user.id)) throw new Error("userToken does not have a user_id or is not matching with user.id");
			}
		};
		this.tokenReady = () => this.loadTokenPromise;
		this.loadToken = () => {
			this.loadTokenPromise = new Promise(async (resolve, reject) => {
				if (this.type === "static") return resolve(this.token);
				if (this.tokenProvider && typeof this.tokenProvider !== "string") {
					try {
						this.token = await this.tokenProvider();
					} catch (e) {
						return reject(new Error(`Call to tokenProvider failed with message: ${e}`, { cause: e }));
					}
					resolve(this.token);
				}
			});
		...
		this.getToken = () => {
			...
			if (this.user && this.user.anon && !this.token) return this.token;
			if (this.secret) return JWTServerToken(this.secret);
		...
		this.type = "static";
		if (this.secret) this.token = JWTServerToken(this.secret);
	}
};
var WSConnectionFallback = class {
	constructor({ client }) {
		...
		this._onlineStatusChanged = (event) => {
			...
			if (event.type === "online" && this.state === "CLOSED") this.connect(true);
		};
		/** @private */
		this._req = async (params, config, retry) => {
			if (!this.cancelToken && !params.close) this.cancelToken = axios.CancelToken.source();
			try {
				const res = await this.client.doAxiosRequest("get", this.client.baseURL.replace(":3030", ":8900") + "/longpoll", void 0, {
					config: {
						...config,
		...
		/** @private */
		this._poll = async () => {
			while (this.state === "CONNECTED") try {
				const data = await this._req({}, { timeout: 3e4 }, true);
				if (data.events?.length) for (let i = 0; i < data.events.length; i++) this.client.dispatchEvent(data.events[i]);
			} catch (error) {
		...
		*/
		this.connect = async (reconnect = false) => {
			...
			try {
				const { event } = await this._req({ json: this.client._buildWSPayload() }, { timeout: 8e3 }, reconnect);
				this._setState("CONNECTED");
				this.connectionID = event.connection_id;
				this.client.dispatchEvent(event);
				this._poll();
				if (reconnect) this.client.recoverState();
				return event;
			} catch (err) {
				this._setState("CLOSED");
				throw err;
		...
		*/
		this.isHealthy = () => !!this.connectionID && this.state === "CONNECTED";
	...
	_setState(state) {
		...
		if (this.state === "CONNECTING" && state === "CONNECTED") this.client.dispatchEvent({
			type: "connection.changed",
			online: true
		});
		if (state === "CLOSED" || state === "DISCONNECTED") this.client.dispatchEvent({
			type: "connection.changed",
			online: false
		});
		this.state = state;
...
var Segment = class {
	...
	verifySegmentId() {
		if (!this.id) throw new Error("Segment id is missing. Either create the segment using segment.create() or set the id during instantiation - const segment = client.segment(id)");
	}
	get() {
...
var Moderation = class {
	...
	*/
	async checkUserProfile(userId, profile) {
		if (!profile.username && !profile.image) throw new Error("Either username or image must be provided");
		const moderationPayload = {};
		if (profile.username) moderationPayload.texts = [profile.username];
		if (profile.image) moderationPayload.images = [profile.image];
	...
	*/
	async upsertModerationRule(rule) {
		return await this.client.post(this.client.baseURL + "/api/v2/moderation/moderation_rule", rule);
	}
	/**
	...
	*/
	async queryModerationRules(filterConditions = {}, sort = [], options = {}) {
		return await this.client.post(this.client.baseURL + "/api/v2/moderation/moderation_rules", {
			filter: filterConditions,
			sort,
			...options
		});
	...
	*/
	async getModerationRule(id) {
		return await this.client.get(this.client.baseURL + "/api/v2/moderation/moderation_rule/" + id);
	}
	/**
	...
	*/
	async deleteModerationRule(id) {
		return await this.client.delete(this.client.baseURL + "/api/v2/moderation/moderation_rule/" + id);
	}
};
...
var ThreadManager = class extends WithSubscriptions {
	constructor({ client }) {
		...
		this.subscribeUnreadThreadsCountChange = () => {
			...
			const unsubscribeFunctions = [
				...
				if (typeof unreadThreadCount2 === "number") this.state.partialNext({ unreadThreadCount: unreadThreadCount2 });
			}).unsubscribe);
			return () => unsubscribeFunctions.forEach((unsubscribe) => unsubscribe());
		};
		...
		this.subscribeNewReplies = () => this.client.on("notification.thread_message_new", (event) => {
			...
			if (!ready) return;
			if (this.threadsById[parentId]) this.state.partialNext({ isThreadOrderStale: true });
			else if (!unseenThreadIds.includes(parentId)) this.state.partialNext({ unseenThreadIds: unseenThreadIds.concat(parentId) });
		}).unsubscribe;
		this.subscribeRecoverAfterConnectionDrop = () => {
			const unsubscribeConnectionDropped = this.client.on("connection.changed", (event) => {
				if (event.online === false) this.state.next((current) => current.lastConnectionDropAt ? current : {
					...current,
			...
			try {
				...
				for (const incomingThread of response.threads) {
					...
					if (existingThread) {
						...
						if (existingThread.hasStaleState) existingThread.hydrateState(incomingThread);
					} else nextThreads.push(incomingThread);
				}
				this.state.next((current) => ({
					...current,
					threads: nextThreads,
	...
	get threadsById() {
		const { threads } = this.state.getLatestValue();
		if (threads === this.threadsByIdGetterCache.threads) return this.threadsByIdGetterCache.threadsById;
	...
	constructor({ client, poll }) {
		this.getInitialStateFromPollResponse = (poll) => {
			...
			const { ownAnswer, ownVotes } = own_votes?.reduce((acc, voteOrAnswer) => {
				if (isVoteAnswer(voteOrAnswer)) acc.ownAnswer = voteOrAnswer;
				else acc.ownVotes.push(voteOrAnswer);
				return acc;
			}, { ownVotes: [] }) ?? { ownVotes: [] };
			return {
		...
		this.upsertOfflineDb = () => {
			this.client.offlineDb?.executeQuerySafely((db) => db.upsertPoll({ poll: mapPollStateToResponse(this) }), { method: "upsertPoll" });
		};
		...
		this.handleVoteCasted = (event) => {
			...
			if (isOwnVote) {
				if (isVoteAnswer(event.poll_vote)) ownAnswer = event.poll_vote;
				else if (event.poll_vote.option_id) ownVotesByOptionId[event.poll_vote.option_id] = event.poll_vote;
			}
			if (isVoteAnswer(event.poll_vote)) latestAnswers = [event.poll_vote, ...latestAnswers];
			else maxVotedOptionIds = getMaxVotedOptionIds(event.poll.vote_counts_by_option);
		...
		this.handleVoteChanged = (event) => {
			...
			if (isOwnVote) {
				if (isVoteAnswer(event.poll_vote)) {
					...
					if (event.poll.enforce_unique_vote) ownVotesByOptionId = { [event.poll_vote.option_id]: event.poll_vote };
					else {
						ownVotesByOptionId = Object.entries(ownVotesByOptionId).reduce((acc, [optionId, vote]) => {
							if (optionId !== event.poll_vote.option_id && vote.id === event.poll_vote.id) return acc;
							acc[optionId] = vote;
							return acc;
						}, {});
						ownVotesByOptionId[event.poll_vote.option_id] = event.poll_vote;
					}
					if (ownAnswer?.id === event.poll_vote.id) ownAnswer = void 0;
					maxVotedOptionIds = getMaxVotedOptionIds(event.poll.vote_counts_by_option);
				}
			} else if (isVoteAnswer(event.poll_vote)) latestAnswers = [event.poll_vote, ...latestAnswers];
			else maxVotedOptionIds = getMaxVotedOptionIds(event.poll.vote_counts_by_option);
			const pollEnrichData = extractPollEnrichedData(event.poll);
			this.state.partialNext({
				...pollEnrichData,
		...
		this.handleVoteRemoved = (event) => {
			...
			if (isVoteAnswer(event.poll_vote)) {
				...
				if (isOwnVote) ownAnswer = void 0;
			} else {
				maxVotedOptionIds = getMaxVotedOptionIds(event.poll.vote_counts_by_option);
				if (isOwnVote && event.poll_vote.option_id) delete ownVotesByOptionId[event.poll_vote.option_id];
			}
			const pollEnrichData = extractPollEnrichedData(event.poll);
			this.state.partialNext({
				...pollEnrichData,
		...
		this.castVote = async (optionId, messageId) => {
			...
			if (max_votes_allowed && max_votes_allowed === Object.keys(ownVotesByOptionId).length) {
				this.client.notifications.addInfo({
					...
					options: { type: "validation:poll:castVote:limit" }
				});
				return;
			}
...
function getMaxVotedOptionIds(voteCountsByOption) {
	let maxVotes = 0;
	let winningOptions = [];
	for (const [id, count] of Object.entries(voteCountsByOption ?? {})) if (count > maxVotes) {
		winningOptions = [id];
		maxVotes = count;
	} else if (count === maxVotes) winningOptions.push(id);
...
function mapPollStateToResponse(poll) {
	const { lastActivityAt, maxVotedOptionIds, ownVotesByOptionId, ownAnswer, ...restState } = poll.data;
...
var PollManager = class extends WithSubscriptions {
	constructor({ client }) {
		...
		this.createPoll = async (poll) => {
			...
			if (!createdPoll.vote_counts_by_option) createdPoll.vote_counts_by_option = {};
		...
		this.setOrOverwriteInCache = (pollResponse, overwriteState) => {
			if (!this.client._cacheEnabled()) return;
			...
			} else if (overwriteState) pollFromCache.reinitializeState(pollResponse);
		};
		this.subscribePollUpdated = () => this.client.on("poll.updated", (event) => {
			if (event.poll?.id) this.fromState(event.poll.id)?.handlePollUpdated(event);
		}).unsubscribe;
		this.subscribePollClosed = () => this.client.on("poll.closed", (event) => {
			if (event.poll?.id) this.fromState(event.poll.id)?.handlePollClosed(event);
		}).unsubscribe;
		this.subscribeVoteCasted = () => this.client.on("poll.vote_casted", (event) => {
			if (event.poll?.id) this.fromState(event.poll.id)?.handleVoteCasted(event);
		}).unsubscribe;
		this.subscribeVoteChanged = () => this.client.on("poll.vote_changed", (event) => {
			if (event.poll?.id) this.fromState(event.poll.id)?.handleVoteChanged(event);
		}).unsubscribe;
		this.subscribeVoteRemoved = () => this.client.on("poll.vote_removed", (event) => {
			if (event.poll?.id) this.fromState(event.poll.id)?.handleVoteRemoved(event);
		}).unsubscribe;
...
var DEFAULT_CHANNEL_MANAGER_PAGINATION_OPTIONS = { offset: 0 };
var mapPredefinedFilterSortToChannelSort = (sort) => (sort ?? []).map(({ direction = 1, field }) => ({ [field]: direction }));
var getResponsePaginationParams = ({ queryChannelsResponse, sort }) => {
	const predefinedFilter = queryChannelsResponse?.predefined_filter;
	if (!predefinedFilter) return {};
	return {
	...
	constructor({ client, eventHandlerOverrides = {}, options = {}, queryChannelsOverride }) {
		...
		this.setChannels = (valueOrFactory) => {
			this.state.next((current) => {
				...
				if (currentChannels === newChannels) return current;
				return {
					...current,
					channels: newChannels
				};
			});
			const { channels, pagination: { filters, options, sort } } = this.state.getLatestValue();
		...
		this.executeChannelsQuery = async (payload, retryCount = 0) => {
			...
			try {
				const queryChannelsResponse = await this.queryChannelsRequest(filters, sort, options, {
					...stateOptions,
					withResponse: true
				});
				...
				if (retryCount >= DEFAULT_QUERY_CHANNELS_RETRY_COUNT) {
					console.warn(err);
					const wrappedError = /* @__PURE__ */ new Error(`Maximum number of retries reached in queryChannels. Last error message is: ${err}`);
					const state = this.state.getLatestValue();
		...
		this.queryChannels = async (filters, sort = [], options = {}, stateOptions = {}) => {
			const { pagination: { isLoading, filters: filtersFromState }, initialized } = this.state.getLatestValue();
			if (isLoading && !this.options.abortInFlightQuery && JSON.stringify(filtersFromState) === JSON.stringify(filters)) return;
		...
		this.loadNext = async () => {
			...
			if (!initialized || isLoadingNext || !hasNext) return;
			try {
				const { offset, limit } = {
					...DEFAULT_CHANNEL_MANAGER_PAGINATION_OPTIONS,
					...options
				...
				const queryChannelsResponse = await this.queryChannelsRequest(filters, sort, options, this.stateOptions);
		...
		this.notificationAddedToChannelHandler = async (event) => {
			...
			if (!type || !this.options.allowNotLoadedChannelPromotionForEvent?.["notification.added_to_channel"]) return;
		...
		this.newMessageHandler = (event) => {
			...
			const considerPinnedChannels = shouldConsiderPinnedChannels(sort);
			if (considerArchivedChannels && isTargetChannelArchived && !filters.archived || considerArchivedChannels && !isTargetChannelArchived && filters.archived || considerPinnedChannels && isTargetChannelPinned || this.options.lockChannelOrder || !targetChannelExistsWithinList && !this.options.allowNotLoadedChannelPromotionForEvent?.["message.new"]) return;
			this.setChannels(promoteChannel({
				channels,
		...
		this.notificationNewMessageHandler = async (event) => {
			...
			if (!channels || considerArchivedChannels && isTargetChannelArchived && !filters.archived || considerArchivedChannels && !isTargetChannelArchived && filters.archived || !this.options.allowNotLoadedChannelPromotionForEvent?.["notification.message_new"]) return;
			this.setChannels(promoteChannel({
				channels,
				channelToMove: channel,
				sort
		...
		this.channelVisibleHandler = async (event) => {
			...
			const considerArchivedChannels = shouldConsiderArchivedChannels(filters);
			const isTargetChannelArchived = isChannelArchived(channel);
			if (!channels || considerArchivedChannels && isTargetChannelArchived && !filters.archived || considerArchivedChannels && !isTargetChannelArchived && filters.archived || !this.options.allowNotLoadedChannelPromotionForEvent?.["channel.visible"]) return;
			this.setChannels(promoteChannel({
				channels,
				channelToMove: channel,
				sort
		...
		this.memberUpdatedHandler = (event) => {
			const { pagination, channels } = this.state.getLatestValue();
			const { filters, sort } = getResponseFiltersAndSort(pagination);
			if (!event.member?.user || event.member.user.id !== this.client.userID || !event.channel_type || !event.channel_id) return;
			const channelType = event.channel_type;
			const channelId = event.channel_id;
			const considerPinnedChannels = shouldConsiderPinnedChannels(sort);
			const considerArchivedChannels = shouldConsiderArchivedChannels(filters);
			...
			if (!channels || !considerPinnedChannels && !considerArchivedChannels || this.options.lockChannelOrder) return;
			const targetChannel = this.client.channel(channelType, channelId);
			const targetChannelIndex = channels.indexOf(targetChannel);
			const targetChannelExistsWithinList = targetChannelIndex >= 0;
			const isTargetChannelPinned = isChannelPinned(targetChannel);
			...
			let lastPinnedChannelIndex = null;
			if (pinnedAtSort === 1 || pinnedAtSort === -1 && !isTargetChannelPinned) lastPinnedChannelIndex = findLastPinnedChannelIndex({ channels: newChannels });
			const newTargetChannelIndex = typeof lastPinnedChannelIndex === "number" ? lastPinnedChannelIndex + 1 : 0;
			if (channels[newTargetChannelIndex] === targetChannel) return;
		...
		this.subscriptionOrOverride = (event) => {
			...
			if (defaultEventHandler && typeof defaultEventHandler === "function") defaultEventHandler(event);
		};
		this.registerSubscriptions = () => {
			if (this.hasSubscriptions) return;
			for (const eventType of Object.keys(channelManagerEventToHandlerMapping)) this.addUnsubscribeFunction(this.client.on(eventType, this.subscriptionOrOverride).unsubscribe);
		};
...
var DURATION_MS = 3e3;
var DEFAULT_NOTIFICATION_MANAGER_CONFIG = { durations: {
	error: DURATION_MS,
	info: DURATION_MS,
	success: DURATION_MS,
	warning: DURATION_MS
...
var NotificationManager = class {
	...
	add({ message, origin, options = {} }) {
		...
		this.store.partialNext({ notifications: this.config.sortComparator ? [...notifications].sort(this.config.sortComparator) : notifications });
		return id;
	}
	...
	remove(id) {
		this.clearTimeout(id);
		this.store.partialNext({ notifications: this.store.getLatestValue().notifications.filter((n) => n.id !== id) });
	}
	clear() {
...
var ReminderTimer = class {
	constructor({ reminder, config }) {
		...
		this.getRefreshIntervalLength = () => {
			...
			let refreshInterval;
			if (distanceFromDeadlineMs === 0) refreshInterval = oneMinute;
			else if (distanceFromDeadlineMs < GROUP_BOUNDS.minute.lower) refreshInterval = distanceFromDeadlineMs;
			else if (distanceFromDeadlineMs <= GROUP_BOUNDS.minute.upper) refreshInterval = oneMinute;
			else if (distanceFromDeadlineMs <= GROUP_BOUNDS.hour.upper) refreshInterval = oneHour;
			else refreshInterval = oneDay;
		...
		this.init = () => {
			...
			if (this.reminder.remindAt?.getTime() + this.stopRefreshBoundaryMs - Date.now() <= 0) {
				this.timeout = null;
				return;
			}
		...
		if (typeof config?.stopRefreshBoundaryMs === "number") this.stopRefreshBoundaryMs = config.stopRefreshBoundaryMs;
	}
};
	...
	constructor({ data, config }) {
		this.setState = (data) => {
			this.state.next((current) => {
				...
				if (newState.remind_at) newState.timeLeftMs = timeLeftMs(newState.remind_at.getTime());
				return newState;
			});
			if (data.remind_at) this.initTimer();
			else if (!data.remind_at) this.clearTimer();
		};
...
var _ReminderManager = class _ReminderManager extends WithSubscriptions {
	constructor({ client, config }) {
		super();
		this.upsertToState = ({ data, overwrite = true }) => {
			if (!this.client._cacheEnabled()) return;
			...
			if (!cachedReminder) {
				...
				this.state.partialNext({ reminders: new Map(this.reminders.set(data.message_id, reminder)) });
			} else if (overwrite) cachedReminder.setState(data);
		...
		this.hydrateState = (messages) => {
			messages.forEach(({ reminder }) => {
				if (reminder) this.upsertToState({ data: reminder });
			});
		};
		this.initTimers = () => {
		...
		this.subscribePaginatorStateUpdated = () => this.paginator.state.subscribeWithSelector(({ items }) => [items], ([items]) => {
			if (!items) return;
			for (const reminder of items) this.upsertToState({ data: reminder });
		});
		this.subscribeConfigStateUpdated = () => this.configState.subscribeWithSelector(({ stopTimerRefreshBoundaryMs }) => ({ stopTimerRefreshBoundaryMs }), ({ stopTimerRefreshBoundaryMs }, previousValue) => {
			if (typeof stopTimerRefreshBoundaryMs === "number" && stopTimerRefreshBoundaryMs !== previousValue?.stopTimerRefreshBoundaryMs) this.reminders.forEach((reminder) => {
				if (reminder.timer) reminder.timer.stopRefreshBoundaryMs = stopTimerRefreshBoundaryMs;
			});
		});
		this.upsertReminder = async (options) => {
			const { messageId } = options;
			if (this.getFromState(messageId)) try {
				return await this.updateReminder(options);
			} catch (error) {
				if (isReminderDoesNotExistError(error)) return await this.createReminder(options);
				throw error;
			}
			else try {
				return await this.createReminder(options);
			} catch (error) {
				if (isReminderExistsError(error)) return await this.updateReminder(options);
				throw error;
			}
		};
	...
	updateConfig(config) {
		if (typeof config.stopTimerRefreshBoundaryMs === "number" && config.stopTimerRefreshBoundaryMs !== this.stopTimerRefreshBoundaryMs) this.reminders.forEach((reminder) => {
...
var StreamChat = class _StreamChat {
	constructor(key, secretOrOptions, options) {
		...
		* @private
		*/
		this._messageComposerSetupState = new StateStore({ setupFunction: null });
		...
		*/
		this.connectUser = async (user, userTokenOrProvider) => {
			if (!user.id) throw new Error("The \"id\" field on the user is missing");
			...
			if (this.userID) throw new Error("Use client.disconnect() before trying to connect as a different user. connectUser was called twice.");
			if ((this._isUsingServerAuth() || this.node) && !this.options.allowServerSideConnect) console.warn("Please do not use connectUser server side. connectUser impacts MAU and concurrent connection usage and thus your bill. If you have a valid use-case, add \"allowServerSideConnect: true\" to the client options to disable this warning.");
			this.userID = user.id;
			...
			try {
				...
				if (this.persistUserOnConnectionFailure) this.closeConnection();
				else this.disconnectUser();
				throw err;
			}
		};
		/**
		...
		*/
		this.closeConnection = async (timeout) => {
			...
			this.offlineDb?.executeQuerySafely(async (db) => {
				if (this.userID) await db.upsertUserSyncStatus({
					userId: this.userID,
		...
		*/
		this.createChannelManager = ({ eventHandlerOverrides = {}, options = {}, queryChannelsOverride }) => new ChannelManager({
			client: this,
		...
		*/
		this.openConnection = () => {
			if (!this.userID) throw Error("User is not set on client, use client.connectUser or client.connectAnonymousUser instead");
			if (this.wsConnection?.isConnecting && this.wsPromise) {
				this.logger("info", "client:openConnection() - connection already in progress", { tags: ["connection", "client"] });
				return this.wsPromise;
			}
			if ((this.wsConnection?.isHealthy || this.wsFallback?.isHealthy()) && this._hasConnectionID()) {
				this.logger("info", "client:openConnection() - openConnection called twice, healthy connection already exists", { tags: ["connection", "client"] });
				return;
			}
		...
		*/
		...
		this._normalizeDate = (before) => {
			...
			if (before === "") throw new Error("Don't pass blank string for since, use null instead if resetting the token revoke");
			return before;
		};
		/**
		...
		*/
		this.disconnectUser = (timeout) => {
			this.logger("info", "client:disconnect() - Disconnecting the client", { tags: ["connection", "client"] });
			delete this.user;
			delete this._user;
			delete this.userID;
			this.anonymous = false;
			const closePromise = this.closeConnection(timeout);
			for (const channel of Object.values(this.activeChannels)) channel._disconnect();
			this.activeChannels = {};
		...
		*/
			...
			try {
				...
				if (e.response) {
					if (e.response.data.code === chatCodes.TOKEN_EXPIRED && !this.tokenManager.isStatic()) {
						if (this.consecutiveFailures > 1) await sleep(retryInterval(this.consecutiveFailures));
		...
		this.dispatchEvent = (event) => {
			...
			if (channel) channel._handleChannelEvent(event);
			this._callClientListeners(event);
			if (channel) channel._callChannelListeners(event);
			postListenerCallbacks.forEach((c) => c());
			this.offlineDb?.executeQuerySafely((db) => db.handleEvent({ event }), { method: `handleEvent;${event.type}` });
		};
		/**
		...
		*/
		this._updateMemberWatcherReferences = (user) => {
			...
			for (const channelID in refMap) {
				...
				if (channel?.state) {
					if (channel.state.members[user.id]) channel.state.members[user.id].user = user;
					if (channel.state.watchers[user.id]) channel.state.watchers[user.id] = user;
					if (channel.state.read[user.id]) channel.state.read[user.id].user = user;
				}
			}
		};
		/**
		...
		*/
		this._updateUserMessageReferences = (user) => {
			...
			for (const channelID in refMap) {
				const channel = this.activeChannels[channelID];
				if (!channel) continue;
				channel.state?.updateUserMessages(user);
			}
		};
		/**
		* @private
		...
		*/
		this._deleteUserMessageReference = (user, hardDelete = false, deletedAt) => {
			...
			for (const channelID in refMap) {
				const channel = this.activeChannels[channelID];
				if (channel) channel.state?.deleteUserMessages(user, hardDelete, deletedAt);
			}
		};
		/**
		* @private
		...
		*/
		this._handleUserEvent = (event) => {
			...
			if (event.type === "user.presence.changed" || event.type === "user.updated") {
				if (event.user.id === this.userID) {
					...
					for (const key in this.user) {
						if (key in event.user || isOwnUserBaseProperty(key)) continue;
						const deleteKey = key;
					...
					for (const key in _user) {
						const updateKey = key;
						if (updateKey in event.user) _user[updateKey] = event.user[updateKey];
					}
					this._user = _user;
					this.user = {
						...user,
			...
			if (event.type === "user.updated") this._updateUserMessageReferences(event.user);
			if (event.type === "user.deleted" && event.user.deleted_at && (event.mark_messages_deleted || event.hard_delete)) this._deleteUserMessageReference(event.user, event.hard_delete, event.user.deleted_at ? new Date(event.user.deleted_at) : null);
		};
		this._callClientListeners = (event) => {
			const client = this;
			const listeners = [];
			if (client.listeners.all) listeners.push(...client.listeners.all);
			if (client.listeners[event.type]) listeners.push(...client.listeners[event.type]);
			for (const listener of listeners) listener(event);
		};
		this.recoverState = async () => {
			this.logger("info", `client:recoverState() - Start of recoverState with connectionID ${this._getConnectionID()}`, { tags: ["connection"] });
			const cids = Object.keys(this.activeChannels);
			if (cids.length && this.recoverStateOnReconnect) {
				this.logger("info", `client:recoverState() - Start the querying of ${cids.length} channels`, { tags: ["connection", "client"] });
				...
				this.dispatchEvent({ type: "connection.recovered" });
			} else this.dispatchEvent({ type: "connection.recovered" });
		...
		*/
		this.getChannelByMembers = (channelType, custom) => {
			...
			if (!tempCid) throw Error("Please specify atleast one member when creating unique conversation");
			for (const key in this.activeChannels) {
				const channel2 = this.activeChannels[key];
				if (channel2.disconnected) continue;
				if (key === tempCid) return channel2;
				if (key.indexOf(`${channelType}:!members-`) === 0) {
					if (Object.keys(channel2.state.members).sort().join(",") === membersStr) return channel2;
				}
			}
			const channel = new Channel(this, channelType, void 0, custom);
			if (this._cacheEnabled()) this.activeChannels[tempCid] = channel;
			return channel;
		};
		/**
		...
		*/
		this.getChannelById = (channelType, channelID, custom) => {
			if (typeof channelID === "string" && ~channelID.indexOf(":")) throw Error(`Invalid channel id ${channelID}, can't contain the : character`);
			...
			if (this._cacheEnabled()) this.activeChannels[channel.cid] = channel;
			return channel;
		};
		/**
		...
		*/
		...
		if (secretOrOptions && isString2(secretOrOptions)) this.secret = secretOrOptions;
		...
		if (this.node && !this.options.httpsAgent) this.options.httpsAgent = new import_https.default.Agent({
			keepAlive: true,
			keepAliveMsecs: 3e3
		});
		this.axiosInstance = axios.create(this.options);
		this.setBaseURL(this.options.baseURL || "https://chat.stream-io-api.com");
		if (typeof process !== "undefined" && "env" in process && process.env.STREAM_LOCAL_TEST_RUN) this.setBaseURL("http://localhost:3030");
		if (typeof process !== "undefined" && "env" in process && process.env.STREAM_LOCAL_TEST_HOST) this.setBaseURL("http://" + process.env.STREAM_LOCAL_TEST_HOST);
	...
	static getInstance(key, secretOrOptions, options) {
		if (!_StreamChat._instance) if (typeof secretOrOptions === "string") _StreamChat._instance = new _StreamChat(key, secretOrOptions, options);
		else _StreamChat._instance = new _StreamChat(key, secretOrOptions);
	...
	*/
	async updateAppSettings(options) {
		...
		if (apn_config?.p12_cert) options = {
			...options,
			apn_config: {
				...apn_config,
	...
	*/
	async revokeUsersToken(userIDs, before) {
		if (before === void 0) before = (/* @__PURE__ */ new Date()).toISOString();
		else before = this._normalizeDate(before);
		const users = [];
		for (const userID of userIDs) users.push({
			id: userID,
			set: { revoke_tokens_issued_before: before }
		});
	...
	*/
	createToken(userID, exp, iat) {
		if (this.secret == null) throw Error(`tokens can only be created server-side using the API Secret`);
		const extra = {};
		if (exp) extra.exp = exp;
		if (iat) extra.iat = iat;
	...
	errorFromResponse(response) {
		return new ErrorFromResponse(typeof response.data.code !== "undefined" ? `StreamChat error code ${response.data.code}: ${response.data.message}` : `StreamChat error HTTP code: ${response.status}`, {
	...
	handleResponse(response) {
		...
		if (isErrorResponse(response)) throw this.errorFromResponse(response);
		return data;
	}
	_handleClientEvent(event) {
		...
		if (event.type === "user.presence.changed" || event.type === "user.updated" || event.type === "user.deleted") this._handleUserEvent(event);
		if (event.type === "user.messages.deleted" && !event.cid && event.user) this._deleteUserMessageReference(event.user, event.hard_delete, event.created_at ? new Date(event.created_at) : null);
		...
		if (event.type === "notification.channel_mutes_updated" && event.me?.channel_mutes) this.mutedChannels = event.me.channel_mutes;
		if (event.type === "notification.mutes_updated" && event.me?.mutes) this.mutedUsers = event.me.mutes;
		if (event.type === "notification.mark_read" && event.unread_channels === 0) Object.keys(this.activeChannels).forEach((activeChannelKey) => this.activeChannels[activeChannelKey].state.unreadCount = 0);
	...
	*/
	async connect() {
		if (!this.userID || !this._user) throw Error("Call connectUser or connectAnonymousUser before starting the connection");
		if (!this.wsBaseURL) throw Error("Websocket base url not set");
		if (!this.clientID) throw Error("clientID is not set");
		if (!this.wsConnection && (this.options.warmUp || this.options.enableInsights)) this._sayHi();
		if (this.options.wsConnection && this.node) {
			this.options.wsConnection.setClient(this);
			this.wsConnection = this.options.wsConnection;
		} else this.wsConnection = new StableWSConnection({ client: this });
		try {
			if (this.wsFallback) return await this.wsFallback.connect();
	...
	*/
	_sayHi() {
		...
		this.doAxiosRequest("get", this.baseURL + "/hi", null, opts).catch((e) => {
			if (this.options.enableInsights) postInsights("http_hi_failed", {
				api_key: this.key,
				err: e,
				client_request_id
			});
	...
	*/
	async queryUsers(filterConditions, sort = [], options = {}) {
		const defaultOptions = { presence: false };
		await this.wsPromise;
		if (!this._hasConnectionID()) defaultOptions.presence = false;
		const data = await this.get(this.baseURL + "/users", { payload: {
			filter_conditions: filterConditions,
			sort: normalizeQuerySort(sort),
			...defaultOptions,
			...options
	...
	*/
	async queryBannedUsers(filterConditions = {}, sort = [], options = {}) {
		return await this.get(this.baseURL + "/query_banned_users", { payload: {
			filter_conditions: filterConditions,
			sort: normalizeQuerySort(sort),
			...options
		} });
	...
	*/
	async queryFutureChannelBans(options = {}) {
		return await this.get(this.baseURL + "/query_future_channel_bans", { payload: options });
	}
	/**
	...
	*/
	async queryMessageFlags(filterConditions = {}, options = {}) {
		return await this.get(this.baseURL + "/moderation/flags/message", { payload: {
			filter_conditions: filterConditions,
			...options
		} });
	}
	...
	*/
	async queryChannelsRequestWithResponse(filterConditions, sort = [], options = {}) {
		...
		await this.wsPromise;
		if (!this._hasConnectionID()) defaultOptions.watch = false;
	...
	*/
	async queryChannelsRequest(filterConditions, sort = [], options = {}) {
		return (await this.queryChannelsRequestWithResponse(filterConditions, sort, options)).channels;
	}
	async queryChannels(filterConditions, sort = [], options = {}, stateOptions = {}) {
		const queryChannelsResponse = await this.queryChannelsRequestWithResponse(filterConditions, sort, options);
		...
		if (channels?.length && this.offlineDb?.upsertChannels) await this.offlineDb.upsertChannels({
			channels,
			isLatestMessagesSet: true
		});
		const hydratedChannels = this.hydrateActiveChannels(channels, stateOptions, options);
		if (stateOptions.withResponse) return {
	...
	*/
	async queryReactions(messageID, filter, sort = [], options = {}) {
		...
		if (this.offlineDb?.getReactions && !options.next) try {
			...
			if (reactionsFromDb) this.dispatchEvent({
				type: "offline_reactions.queried",
				offlineReactions: reactionsFromDb
			});
		} catch (e) {
			this.logger("warn", "An error has occurred while querying offline reactions", { error: e });
		}
		await this.wsPromise;
			...
			if (updatedMessagesSet) {
				updatedMessagesSet.pagination = {
					...
					...messageSetPagination({
						...
						filteredReturnedPage: channelState.messages.filter((m) => !filteredMessageIds.includes(m.id)),
						logger: this.logger
					})
				};
	...
	*/
	setLocalDevice(device) {
		if (this.wsConnection?.isConnecting && this.wsPromise || (this.wsConnection?.isHealthy || this.wsFallback?.isHealthy()) && this._hasConnectionID()) throw new Error("you can only set device before opening a websocket connection");
	...
	*/
	...
	_addChannelConfig({ cid, config }) {
		if (this._cacheEnabled()) this.configs[cid] = config;
	}
	channel(channelType, channelIDOrCustom, custom = {}) {
		...
		if (~channelType.indexOf(":")) throw new Error(`Invalid channel group ${channelType}, can't contain the : character`);
		if (channelIDOrCustom && typeof channelIDOrCustom === "object") return this.getChannelByMembers(channelType, channelIDOrCustom);
		if (!channelIDOrCustom && typeof custom === "object" && custom.members?.length) return this.getChannelByMembers(channelType, custom);
		if (!channelIDOrCustom) return new Channel(this, channelType, void 0, custom);
	...
	*/
	async upsertUsers(users) {
		...
		for (const userObject of users) {
			if (!userObject.id) throw Error("User ID is required when updating a user");
	...
	*/
	async partialUpdateUsers(users) {
		for (const userObject of users) if (!userObject.id) throw Error("User ID is required when updating a user");
	...
	*/
	async restoreUsers(user_ids) {
		return await this.post(this.baseURL + `/users/restore`, { user_ids });
	}
	/**
	...
	*/
	...
	async blockUser(blockedUserID, user_id) {
		...
		if (this._cacheEnabled()) this.blockedUsers.next(({ userIds }) => ({ userIds: userIds.concat(blockedUserID) }));
		return result;
	}
	async getBlockedUsers(user_id) {
		const result = await this.get(this.baseURL + "/users/block", { ...user_id ? { user_id } : {} });
		if (this._cacheEnabled()) this.blockedUsers.partialNext({ userIds: result.blocks.map(({ blocked_user_id }) => blocked_user_id) });
		return result;
	}
	async unBlockUser(blockedUserID, userID) {
		...
		if (this._cacheEnabled()) this.blockedUsers.next(({ userIds }) => ({ userIds: userIds.filter((id) => id !== blockedUserID) }));
		return result;
	}
	/** getSharedLocations
	*
	...
	*/
	async getSharedLocations() {
		return await this.get(this.baseURL + `/users/live_locations`);
	}
	...
	*/
	userMuteStatus(targetID) {
		if (!this.user || !this.wsPromise) throw new Error("Make sure to await connectUser() first.");
		for (let i = 0; i < this.mutedUsers.length; i += 1) if (this.mutedUsers[i].target.id === targetID) return true;
		return false;
	}
	/**
	...
	*/
	_normalizeExpiration(timeoutOrExpirationDate) {
		...
		} else if (isString2(timeoutOrExpirationDate)) pinExpires = timeoutOrExpirationDate;
		else if (timeoutOrExpirationDate instanceof Date) pinExpires = timeoutOrExpirationDate.toISOString();
		return pinExpires;
	}
	/**
	...
	*/
	_validateAndGetMessageId(messageOrMessageId, errorText) {
		let messageId;
		if (typeof messageOrMessageId === "string") messageId = messageOrMessageId;
		else {
			if (!messageOrMessageId.id) throw Error(errorText);
	...
	*/
	pinMessage(messageOrMessageId, timeoutOrExpirationDate, pinnedBy, pinnedAt) {
		...
		return this.partialUpdateMessage(messageId, { set: {
			pinned: true,
	...
	*/
	unpinMessage(messageOrMessageId, userId) {
		const messageId = this._validateAndGetMessageId(messageOrMessageId, "Please specify the message id when calling unpinMessage");
		return this.partialUpdateMessage(messageId, { set: { pinned: false } }, userId);
	}
	/**
	...
	*/
	async updateMessage(message, partialUserOrUserId, options) {
		if (!message.id) throw Error("Please specify the message.id when calling updateMessage");
		const messageId = message.id;
		try {
			if (this.offlineDb) return await this.offlineDb.queueTask({ task: {
	...
	async _updateMessage(message, partialUserOrUserId, options) {
		...
		if (typeof partialUserOrUserId === "string") payload.user_id = partialUserOrUserId;
		else if (typeof partialUserOrUserId?.id === "string") payload.user_id = partialUserOrUserId.id;
	...
	*/
	async partialUpdateMessage(id, partialMessageObject, partialUserOrUserId, options) {
		if (!id) throw Error("Please specify the message.id when calling partialUpdateMessage");
		let user = void 0;
		if (typeof partialUserOrUserId === "string") user = { id: partialUserOrUserId };
		else if (typeof partialUserOrUserId?.id === "string") user = { id: partialUserOrUserId.id };
	...
	*/
	async ephemeralUpdateMessage(messageId, partialMessageObject, partialUserOrUserId, options) {
		...
		return await this.patch(`${this.baseURL}/messages/${encodeURIComponent(messageId)}/ephemeral`, {
			...partialMessageObject,
			...options,
			user
		});
	...
	*/
	async deleteMessage(messageID, optionsOrHardDelete) {
		let options = {};
		if (typeof optionsOrHardDelete === "boolean") options = optionsOrHardDelete ? { hardDelete: true } : {};
		else if (optionsOrHardDelete?.deleteForMe) options = { deleteForMe: true };
		else if (optionsOrHardDelete?.hardDelete) options = { hardDelete: true };
		try {
			if (this.offlineDb) {
				if (options.hardDelete) await this.offlineDb.hardDeleteMessage({ id: messageID });
				else await this.offlineDb.softDeleteMessage({
					id: messageID,
					deleteForMe: options.deleteForMe
				});
				return await this.offlineDb.queueTask({ task: {
					messageId: messageID,
	...
	async _deleteMessage(messageID, optionsOrHardDelete) {
		...
		if (deleteForMe) params = {
			...params,
			delete_for_me: true
		};
		const result = await this.delete(this.baseURL + `/messages/${encodeURIComponent(messageID)}`, params);
		if (deleteForMe) {
	...
	*/
	async queryThreads(options = {}) {
		...
		const requestBody = { ...optionsWithDefaults };
		if (optionsWithDefaults.filter && Object.keys(optionsWithDefaults.filter).length > 0) requestBody.filter = optionsWithDefaults.filter;
		if (optionsWithDefaults.sort && (Array.isArray(optionsWithDefaults.sort) ? optionsWithDefaults.sort.length > 0 : Object.keys(optionsWithDefaults.sort).length > 0)) requestBody.sort = normalizeQuerySort(optionsWithDefaults.sort);
	...
	*/
	async getThread(messageId, options = {}) {
		if (!messageId) throw new Error("Please specify the messageId when calling getThread");
		const optionsWithDefaults = {
	...
	*/
	async partialUpdateThread(messageId, partialThreadObject) {
		if (!messageId) throw Error("Please specify the message id when calling partialUpdateThread");
		...
		}) if (reservedThreadFields.includes(key)) throw Error(`You cannot set ${key} field on Thread object. ${key} is reserved for server-side use. Please omit ${key} from your set object.`);
	...
	getUserAgent() {
		...
		let userAgentString = "";
		if (this.sdkIdentifier) userAgentString = `stream-chat-${this.sdkIdentifier.name}-v${this.sdkIdentifier.version}-llc-v${version}`;
		else userAgentString = `stream-chat-js-v${version}-${this.node ? "node" : "browser"}`;
	...
	*/
	...
	_enrichAxiosOptions(options = {
		...
		if (!options.headers?.["x-client-request-id"]) options.headers = {
			...options.headers,
	...
	_startCleaning() {
		const that = this;
		if (this.cleaningIntervalRef != null) return;
		this.cleaningIntervalRef = setInterval(() => {
			for (const channel of Object.values(that.activeChannels)) channel.clean();
		}, 500);
	}
	/**
	...
	*/
	verifyAndParseWebhook(rawBody, signature) {
		if (!this.secret) throw new InvalidWebhookError("cannot verify webhook signature without an API secret on the client");
	...
	*/
	createPermission(permissionData) {
		return this.post(`${this.baseURL}/permissions`, { ...permissionData });
	}
	...
	*/
	async sendUserCustomEvent(targetUserID, event) {
		return await this.post(`${this.baseURL}/users/${encodeURIComponent(targetUserID)}/event`, { event });
	}
	/**
	...
	*/
	...
	campaign(idOrData, data) {
		if (idOrData && typeof idOrData === "object") return new Campaign(this, null, idOrData);
	...
	*/
	...
	segment(type, idOrData, data) {
		if (typeof idOrData === "string") return new Segment(this, type, idOrData, data);
		return new Segment(this, type, null, idOrData);
	}
	validateServerSideAuth() {
		if (!this.secret) throw new Error("This feature can be used server-side only. Please initialize the client with a secret to use this feature.");
	}
	/**
	...
	*/
	async deleteUsers(user_ids, options = {}) {
		if (typeof options.user !== "undefined" && ![
			...
			"hard",
			"pruning"
		].includes(options.user)) throw new Error("Invalid delete user options. user must be one of [soft hard pruning]");
		if (typeof options.conversations !== "undefined" && !["soft", "hard"].includes(options.conversations)) throw new Error("Invalid delete user options. conversations must be one of [soft hard]");
		if (typeof options.messages !== "undefined" && ![
			"soft",
			"hard",
			"pruning"
		].includes(options.messages)) throw new Error("Invalid delete user options. messages must be one of [soft hard pruning]");
	...
	*/
	async _createImportURL(filename) {
		return await this.post(this.baseURL + `/import_urls`, { filename });
	}
	/**
	...
	*/
	async upsertPushProvider(pushProvider) {
		return await this.post(this.baseURL + `/push_providers`, { push_provider: pushProvider });
	}
	/**
	...
	*/
	closePoll(id, userId) {
		return this.partialUpdatePoll(id, { set: { is_closed: true } }, userId);
	}
	/**
	...
	*/
	addPollAnswer(messageId, pollId, answerText, userId) {
		return this.castPollVote(messageId, pollId, { answer_text: answerText }, userId);
	}
	async removePollVote(messageId, pollId, voteId, userId) {
		return await this.delete(this.baseURL + `/messages/${encodeURIComponent(messageId)}/polls/${encodeURIComponent(pollId)}/vote/${encodeURIComponent(voteId)}`, { ...userId ? { user_id: userId } : {} });
	}
	/**
	* Queries polls
	* @param filter
	...
	*/
	async deleteReminder(messageId, userId) {
		return await this.delete(`${this.baseURL}/messages/${messageId}/reminders`, userId ? { user_id: userId } : {});
	}
	/**
	...
	*/
	async queryTeamUsageStats(options = {}) {
		return await this.post(`${this.baseURL}/stats/team_usage`, options);
	}
	/**
	...
	*/
	async updateLocation(location) {
		return await this.put(this.baseURL + `/users/live_locations`, location);
	}
	/**
	...
	*/
	uploadFile(uri, name, contentType, user, axiosRequestConfig) {
		return this.sendFile(`${this.baseURL}/uploads/file`, uri, name, contentType, user, axiosRequestConfig);
	}
	/**
	...
	*/
	uploadImage(uri, name, contentType, user, axiosRequestConfig) {
		return this.sendFile(`${this.baseURL}/uploads/image`, uri, name, contentType, user, axiosRequestConfig);
	}
	/**
	...
	*/
	async updateChannelsBatch(payload) {
		return await this.put(this.baseURL + `/channels/batch`, payload);
	}
	/**
	...
	*/
	async createPredefinedFilter(options) {
		this.validateServerSideAuth();
		return await this.post(`${this.baseURL}/predefined_filters`, options);
	}
	/**
	...
	*/
	async getPredefinedFilter(name) {
		this.validateServerSideAuth();
		return await this.get(`${this.baseURL}/predefined_filters/${encodeURIComponent(name)}`);
	}
	/**
	...
	*/
	async updatePredefinedFilter(name, options) {
		this.validateServerSideAuth();
		return await this.put(`${this.baseURL}/predefined_filters/${encodeURIComponent(name)}`, options);
	}
	/**
	...
	*/
	async deletePredefinedFilter(name) {
		this.validateServerSideAuth();
		return await this.delete(`${this.baseURL}/predefined_filters/${encodeURIComponent(name)}`);
	}
	/**
	...
	*/
	async listPredefinedFilters(options = {}) {
		...
		return await this.get(`${this.baseURL}/predefined_filters`, {
			...paginationOptions,
	...
	*/
	async setRetentionPolicy(policy, maxAgeHours) {
		this.validateServerSideAuth();
		return await this.post(this.baseURL + "/retention_policy", {
			policy,
	...
	*/
	async deleteRetentionPolicy(policy) {
		this.validateServerSideAuth();
		return await this.post(this.baseURL + "/retention_policy/delete", { policy });
	}
	/**
	...
	*/
	async getRetentionPolicyRuns(options = {}) {
		this.validateServerSideAuth();
		return await this.post(this.baseURL + "/retention_policy/runs", options);
	}
};
var EVENT_MAP = {
...
var AllowAll = new Permission("Allow all", 999, AnyResource, AnyRole, false, Allow);
var DenyAll = new Permission("Deny all", 1, AnyResource, AnyRole, false, Deny);
var BuiltinRoles = {
	Admin: "admin",
	Anonymous: "anonymous",
		...
		*/
		this.init = async () => {
			try {
				...
				if (this.connectionChangedListener) this.connectionChangedListener.unsubscribe();
				this.connectionChangedListener = this.client.on("connection.changed", async (event) => {
					if (event.online) {
						await this.syncAndExecutePendingTasks();
						await this.invokeSyncStatusListeners(true);
					} else await this.invokeSyncStatusListeners(false);
				});
			} catch (error) {
		...
		*/
		this.sync = async () => {
			...
			try {
				...
				const lastSyncedAt = await this.offlineDb.getLastSyncedAt({ userId: this.client.user.id });
				if (lastSyncedAt) {
					const lastSyncedAtDate = new Date(lastSyncedAt);
					if (Math.floor(((/* @__PURE__ */ new Date()).getTime() - lastSyncedAtDate.getTime()) / (1e3 * 60 * 60 * 24)) > 30) await this.offlineDb.resetDB();
					else {
						const queryPromises = (await this.client.sync(cids, lastSyncedAtDate.toISOString())).events.map((event) => this.offlineDb.handleEvent({
							event,
							execute: false
						}));
						const queries = (await Promise.all(queryPromises)).flat();
						if (queries.length) await this.offlineDb.executeSqlBatch(queries);
					}
				}
				...
				if (isAxiosError(e) && e.code === "ECONNABORTED") return;
				if (e.response?.data?.code === 23) return;
	...
	constructor({ client }) {
		...
		*/
		this.init = async (userId) => {
			try {
				if (!this.shouldInitialize(userId)) if (await this.initializeDB()) {
					await this.syncManager.init();
					this.state.partialNext({
						initialized: true,
						userId
					});
				} else this.state.partialNext({ initialized: false });
			} catch (error) {
				this.state.partialNext({
					initialized: false,
					userId: void 0
		...
		*/
		this.queriesWithChannelGuard = async ({ event, execute = true, forceUpdate = false }, createQueries) => {
			const channelFromEvent = event.channel;
			const cid = event.cid || channelFromEvent?.cid;
			const type = event.type;
			if (!cid) return await createQueries(execute);
			if (forceUpdate || !await this.channelExists({ cid })) {
				...
				if (!channelData && event.channel_type && event.channel_id) {
					...
					if (channelFromState.initialized && !channelFromState.disconnected) channelData = channelFromState.data;
				}
				if (channelData) {
					...
					if (channelQuery) {
						...
						if (execute) await this.executeSqlBatch(newQueries);
						return newQueries;
					} else {
						console.warn(`Couldn't create channel queries on ${type} event for an initialized channel that is not in DB, skipping event`, { event });
						return [];
					}
				} else {
					console.warn(`Received ${type} event for a non initialized channel that is not in DB, skipping event`, { event });
					return [];
				}
			}
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleNewMessage = async ({ event, execute = true }) => {
			const client = this.client;
			const { cid, message, user } = event;
			if (!message || message.parent_id && !message.show_in_channel) return [];
			...
			if (execute) await this.executeSqlBatch(finalQueries);
			return finalQueries;
		};
		/**
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleDeleteMessage = async ({ event, execute = true }) => {
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleRemoveMessage = async ({ messageId, execute = true }) => {
			...
			if (execute) await this.executeSqlBatch(queries);
			return queries;
		};
		/**
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleRead = async ({ event, unreadMessages, execute = true }) => {
			const { received_at: last_read, last_read_message_id, unread_messages = 0, user, cid } = event;
			const overriddenUnreadMessages = unreadMessages ?? unread_messages;
			if (user?.id && cid) return await this.queriesWithChannelGuard({
				event,
				execute
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleMemberEvent = async ({ event, execute = true }) => {
			const { member, cid, type } = event;
			if (member && cid) return await this.queriesWithChannelGuard({
				event,
				execute,
				forceUpdate: true
			}, async (executeOverride) => {
				if (type === "member.removed") return await this.deleteMember({
					member,
					cid,
					execute: executeOverride
				});
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleMessageUpdatedEvent = async ({ event, execute = true }) => {
			const { message } = event;
			if (message && !message.parent_id) return await this.queriesWithChannelGuard({
				event,
				execute
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleChannelVisibilityEvent = async ({ event, execute = true }) => {
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleChannelTruncatedEvent = async ({ event, execute = true }) => {
			const { channel } = event;
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleReactionEvent = async ({ event, execute = true }) => {
			const { type, message, reaction } = event;
			if (!(message && reaction)) return [];
			const getReactionMethod = (type2) => {
				switch (type2) {
					case "reaction.new": return this.insertReaction;
					case "reaction.deleted": return this.deleteReaction;
					case "reaction.updated": return this.updateReaction;
					default: throw new Error(`You are trying to handle a non-reaction event (${type2}) through the reaction DB api.`);
				}
			};
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleDraftEvent = async ({ event, execute = true }) => {
			const { cid, draft, type } = event;
			if (!draft) return [];
			if (type === "draft.updated") return await this.upsertDraft({
				draft,
				execute
			});
		...
		* @param execute - whether to immediately execute the operation.
		*/
		this.handleEvent = async ({ event, execute = true }) => {
			const { type, channel } = event;
			if (type.startsWith("reaction")) return await this.handleReactionEvent({
				event,
				execute
			});
			if (type === "message.new") return await this.handleNewMessage({
				event,
				execute
			});
			if (type === "message.deleted") return await this.handleDeleteMessage({
				event,
				execute
			});
			if (type === "message.updated" || type === "message.undeleted") return this.handleMessageUpdatedEvent({
				event,
				execute
			});
			if (type === "message.read" || type === "notification.mark_read") return this.handleRead({
				event,
				unreadMessages: 0,
				execute
			});
			if (type === "notification.mark_unread") return this.handleRead({
				event,
				execute
			});
			if (type.startsWith("member.")) return await this.handleMemberEvent({
				event,
				execute
			});
			if (type === "channel.hidden" || type === "channel.visible") return await this.handleChannelVisibilityEvent({
				event,
				execute
			});
			if (type === "draft.updated" || type === "draft.deleted") return await this.handleDraftEvent({
				event,
				execute
			});
			if ((type === "channel.updated" || type === "notification.message_new" || type === "notification.added_to_channel") && channel) return await this.upsertChannelData({
				channel,
				execute
			});
			if ((type === "channel.deleted" || type === "notification.channel_deleted" || type === "notification.removed_from_channel") && channel) return await this.deleteChannel({
				cid: channel.cid,
				execute
			});
			if (type === "channel.truncated") return await this.handleChannelTruncatedEvent({
				event,
				execute
			});
			return [];
		...
		*/
		this.queueTask = async ({ task }) => {
			const attemptTaskExecution = async () => {
				if (!this.client.wsConnection?.isHealthy) throw new OfflineError("Cannot execute task because the connection has been lost.", { type: "connection:lost" });
				return await this.executeTask({ task });
			};
			try {
				return await attemptTaskExecution();
			} catch (e) {
				if (!this.shouldSkipQueueingTask(e)) await this.handleAddPendingTask({ task });
				throw e;
			}
		};
		/**
		...
		*/
		...
		this.mergeFailedMessageUpdateIntoPendingSendMessage = ({ editedMessage, pendingMessage }) => {
			const normalizedEditedMessageSource = { ...editedMessage };
			if (editedMessage.status === "failed") delete normalizedEditedMessageSource.message_text_updated_at;
			const normalizedEditedMessage = localMessageToNewMessagePayload(normalizedEditedMessageSource);
		...
		this.handleOfflineFailedUpdateMessagePendingTask = async (task) => {
			...
			const pendingSendMessageTask = (await this.getPendingTasks({ messageId: message.id })).find(this.isPendingSendMessageTask);
			if (!pendingSendMessageTask) return;
		...
		*/
		this.handleAddPendingTask = async ({ task }) => {
			if (task.type === "update-message" && !isMessageUpdateReplayable(task.payload[0])) return;
		...
		*/
		this.executeTask = async ({ task }, isPendingTask = false) => {
			if (task.type === "update-message") return await this.client._updateMessage(...task.payload);
			if (task.type === "delete-message") return await this.client._deleteMessage(...task.payload);
			...
			if (channelType && channelId) {
				...
				if (task.type === "create-draft") return await channel._createDraft(...task.payload);
				if (task.type === "delete-draft") return await channel._deleteDraft(...task.payload);
				if (task.type === "send-message") {
					...
					if (isPendingTask && newMessage) {
						if (newMessage?.parent_id) this.client.threads.threadsById[newMessage.parent_id]?.upsertReplyLocally({
							message: newMessage,
		...
		this.executePendingTasks = async () => {
			...
			for (const task of queue) {
				...
				try {
					...
					const error = e;
					if (!this.shouldSkipQueueingTask(error)) continue;
				}
				await this.deletePendingTask({ id: task.id });
			}
		};
		this.client = client;
...
var isExpiredLocation = (location) => {
	return new Date(location.end_at).getTime() < Date.now();
};
function isValidLiveLocationMessage(message) {
	if (!message || message.type === "deleted" || !message.shared_location?.end_at) return false;
...
var _LiveLocationManager = class _LiveLocationManager extends WithSubscriptions {
	constructor({ client, getDeviceId, watchLocation }) {
		if (!client.userID) throw new Error("Live-location sharing is reserved for client-side use only");
		super();
		this.registerSubscriptions = () => {
			this.incrementRefCount();
			if (this.hasSubscriptions) return;
	...
	get deviceId() {
		if (!this._deviceId) this._deviceId = this.getDeviceId();
		return this._deviceId;
	}
	async assureStateInit() {
		...
		this.state.next({
			messages: new Map(active_live_locations.filter((location) => !isExpiredLocation(location)).map((location) => [location.message_id, {
				...location,
	...
	subscribeTargetMessagesChange() {
		...
		const unsubscribe = this.state.subscribeWithSelector(({ messages }) => ({ messages }), ({ messages }) => {
			if (!messages.size) {
				unsubscribeWatchLocation?.();
				unsubscribeWatchLocation = null;
			} else if (messages.size && !unsubscribeWatchLocation) unsubscribeWatchLocation = this.subscribeWatchLocation();
		});
		return () => {
			unsubscribe();
			unsubscribeWatchLocation?.();
	...
	subscribeWatchLocation() {
		...
		return this.watchLocation(({ latitude, longitude }) => {
	...
	subscribeLiveLocationSharingUpdates() {
		const subscriptions = [...[
			...
			if (!event.message) return;
			if (event.type === "live_location_sharing.started") this.registerMessage(event.message);
			else if (event.type === "message.updated") {
				if (this.messages.has(event.message.id) && !isValidLiveLocationMessage(event.message)) this.unregisterMessages([event.message.id]);
				this.registerMessage(event.message);
			} else this.unregisterMessages([event.message.id]);
	...
	registerMessage(message) {
		if (!this.client.userID || message?.user?.id !== this.client.userID || !isValidLiveLocationMessage(message)) return;
	...
	unregisterMessages(messageIds) {
		...
		const newMessages = new Map(Array.from(messages).filter(([messageId, location]) => {
		...
		this.state.partialNext({ messages: newMessages });
	}
};
...
var FixedSizeQueueCache = class {
	...
	*/
	add(key, value) {
		if (this.keys.indexOf(key) > -1) this.keys.splice(this.keys.indexOf(key), 1);
		else if (this.keys.length >= this.size) {
			const itemKey = this.keys.shift();
			if (itemKey) {
				const item = this.peek(itemKey);
				if (item) this.dispose?.(itemKey, item);
...
//#endregion
export { AbstractOfflineDB, Allow, AllowAll, AnyResource, AnyRole, AttachmentManager, AttachmentPostUploadMiddlewareExecutor, AttachmentPreUploadMiddlewareExecutor, BasePaginator, BaseSearchSource, BaseSearchSourceSync, BuiltinPermissions, BuiltinRoles, Campaign, Channel, ChannelBatchUpdater, ChannelManager, ChannelSearchSource, ChannelState, CheckSignature, ClientState, CommandSearchSource, CustomDataManager, DEFAULT_ATTACHMENT_MANAGER_CONFIG, DEFAULT_CHANNEL_MANAGER_OPTIONS, DEFAULT_CHANNEL_MANAGER_PAGINATION_OPTIONS, DEFAULT_COMPOSER_CONFIG, DEFAULT_LINK_PREVIEW_MANAGER_CONFIG, DEFAULT_LOCATION_COMPOSER_CONFIG, DEFAULT_PAGINATION_OPTIONS, DEFAULT_REMINDER_MANAGER_CONFIG, DEFAULT_STOP_REFRESH_BOUNDARY_MS, DEFAULT_TEXT_COMPOSER_CONFIG, Deny, DenyAll, DevToken, EVENT_MAP, ErrorFromResponse, FilterBuilder, FixedSizeQueueCache, InsightMetrics, InvalidWebhookError, InvalidWebhookErrorMessages, JWTServerToken, JWTUserToken, LinkPreviewStatus, LinkPreviewsManager, LiveLocationManager, LocationComposer, MAX_POLL_OPTIONS, MODERATION_ENTITY_TYPES, MaxPriority, MentionsSearchSource, MergedStateStore, MessageComposer, MessageComposerMiddlewareExecutor, MessageDeliveryReporter, MessageDraftComposerMiddlewareExecutor, MessageReceiptsTracker, MessageSearchSource, MiddlewareExecutor, MinPriority, Moderation, NotificationManager, OfflineDBSyncManager, OfflineError, Permission, Poll, PollComposer, PollComposerCompositionMiddlewareExecutor, PollComposerStateMiddlewareExecutor, PollManager, Product, Reminder, ReminderManager, ReminderPaginator, ReminderTimer, SearchController, Segment, StableWSConnection, StateStore, StreamChat, THREAD_MANAGER_INITIAL_STATE, TextComposer, TextComposerMiddlewareExecutor, Thread, ThreadManager, TokenManager, UPDATE_LIVE_LOCATION_REQUEST_MIN_THROTTLE_TIMEOUT, UploadManager, UserFromToken, UserSearchSource, VALID_MAX_VOTES_VALUE_REGEX, VotingVisibility, accentsMap, buildWsFatalInsight, buildWsSuccessAfterFailureInsight, calculateLevenshtein, channelManagerEventToHandlerMapping, chatCodes, createActiveCommandGuardMiddleware, createAttachmentsCompositionMiddleware, createBlockedAttachmentUploadNotificationMiddleware, createCommandEffectsMiddleware, createCommandInjectionMiddleware, createCommandStringExtractionMiddleware, createCommandsMiddleware, createCompositionDataCleanupMiddleware, createCompositionValidationMiddleware, createCustomDataCompositionMiddleware, createDraftAttachmentsCompositionMiddleware, createDraftCommandInjectionMiddleware, createDraftCompositionValidationMiddleware, createDraftCustomDataCompositionMiddleware, createDraftLinkPreviewsCompositionMiddleware, createDraftMessageComposerStateCompositionMiddleware, createDraftTextComposerCompositionMiddleware, createFileFromBlobs, createLinkPreviewsCompositionMiddleware, createMentionsMiddleware, createMessageComposerStateCompositionMiddleware, createPollComposerStateMiddleware, createPostUploadAttachmentEnrichmentMiddleware, createSharedLocationCompositionMiddleware, createTextComposerCompositionMiddleware, createTextComposerPreValidationMiddleware, createUploadConfigCheckMiddleware, createUploadErrorHandlerMiddleware, decodeBase64, decodeSnsPayload, decodeSqsPayload, defaultPollFieldBlurEventValidators, defaultPollFieldChangeEventValidators, encodeBase64, ensureIsLocalAttachment, escapeCommandRegExp, extractPollData, extractPollEnrichedData, formatMessage, generateFileName, getAttachmentTypeFromMimeType, getCompleteCommandInString, getExtensionFromMimeType, getRawCommandName, getTokenizedSuggestionDisplayName, getTriggerCharWithToken, gunzipPayload, insertItemWithTrigger, isAudioAttachment, isBlobButNotFile, isFile, isFileAttachment, isFileList, isFileReference, isGiphyAttachment, isImageAttachment, isImageFile, isLocalAttachment, isLocalAudioAttachment, isLocalFileAttachment, isLocalImageAttachment, isLocalUploadAttachment, isLocalVideoAttachment, isLocalVoiceRecordingAttachment, isOwnUser, isPatch, isScrapedContent, isSharedLocationResponse, isTargetedOptionTextUpdate, isUploadedAttachment, isVideoAttachment, isVoiceRecordingAttachment, isVoteAnswer, localMessageToNewMessagePayload, logChatPromiseExecution, mapPollStateToResponse, notifyCommandDisabled, parseEvent, parseSns, parseSqs, pollCompositionStateProcessors, pollStateChangeValidators, postInsights, promoteChannel, readFileAsArrayBuffer, removeDiacritics, replaceWordWithEntity, stripCommandFromText, textIsEmpty, timeLeftMs, verifyAndParseWebhook, verifySignature };

//# sourceMappingURL=stream-chat.js.map